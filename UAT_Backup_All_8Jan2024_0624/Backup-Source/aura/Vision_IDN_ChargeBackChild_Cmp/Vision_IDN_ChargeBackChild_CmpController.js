({
    doInit: function(component, event, helper){
        console.log('xAxisLabels cmp3 v2====>'+component.get("v.xAxisLabels"))
              ...
              var salesPrice = (rec.Contract_Sales_Indirect_Price__c != null || rec.Contract_Sales_Indirect_Price__c != undefined) ? rec.Contract_Sales_Indirect_Price__c : 0;
               var units = (rec.contract_units_paid_qty__c != null || rec.contract_units_paid_qty__c != undefined) ? rec.contract_units_paid_qty__c : 0;
                                       if(rec.Month_Year__c == component.get("v.firstColumnKey")){
                                           firstColSalesSummary += Math.round((salesPrice + Number.EPSILON) * 100) / 100;//Math.round(salesPrice);
                                           firstColUnitsSummary +=  Math.round(units);
                                           currentfirstc1productFamilyList.push(rec.H1_revised_product_family__c);
                                           
                                       }
                                       if(rec.Month_Year__c == component.get("v.secondColumnKey")){
                                           secColSalesSummary += Math.round((salesPrice + Number.EPSILON) * 100) / 100;//Math.round(salesPrice);
                                           secColUnitsSummary +=  Math.round(units);
                                           currentsecc2productFamilyList.push(rec.H1_revised_product_family__c);
                                           
                                       }
                                       if(rec.Month_Year__c == component.get("v.thirdColumnKey")){
                                           thirdColSalesSummary += Math.round((salesPrice + Number.EPSILON) * 100) / 100;//Math.round(salesPrice);
                                           thirdColUnitsSummary +=  Math.round(units);
                                           currentthirdc3productFamilyList.push(rec.H1_revised_product_family__c);
                                           
                                       }
                                       if(rec.Month_Year__c == component.get("v.fourthColumnKey")){
                                           fourthColSalesSummary += Math.round((salesPrice + Number.EPSILON) * 100) / 100;//Math.round(salesPrice);
                                           fourthColUnitsSummary +=  Math.round(units);
                                           currentfourthc4productFamilyList.push(rec.H1_revised_product_family__c);
                                           
                                       }
                                       if(rec.Month_Year__c == component.get("v.fifthColumnKey")){
                                           fifthColSalesSummary += Math.round((salesPrice + Number.EPSILON) * 100) / 100;//Math.round(salesPrice);
                                           fifthColUnitsSummary +=  Math.round(units);
                                           currentfifthc5productFamilyList.push(rec.H1_revised_product_family__c);
                                          
                                       }
                                       if(rec.Month_Year__c == component.get("v.sixthColumnKey")){
                                           sixthColSalesSummary += Math.round((salesPrice + Number.EPSILON) * 100) / 100;//Math.round(salesPrice);
                                           sixthColUnitsSummary +=  Math.round(units);
                                           currentsixthc6productFamilyList.push(rec.H1_revised_product_family__c);
	...
	handleClick1: function(component, event, helper){
      ...
      //helper.getFullDataIndividual(component, event, helper);
