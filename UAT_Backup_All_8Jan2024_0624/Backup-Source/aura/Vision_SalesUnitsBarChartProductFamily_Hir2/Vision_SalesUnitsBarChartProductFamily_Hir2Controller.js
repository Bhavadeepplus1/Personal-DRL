({
	 initCharts: function(component, event, helper){
        ...
        console.log('allmaterialDescriptionlist<<<'+JSON.stringify(allmaterialDescriptionlist))
        ...
        prodFamilyRelatedList.forEach(function(rec){
            ...
            if(materialdescriptionmapObj.hasOwnProperty(rec.T2_revised_product_family__c)){
                console.log('inside if==>'+rec.T2_revised_product_family__c);
                var relatedList = materialdescriptionmapObj[rec.T2_revised_product_family__c];
                relatedList.push(rec);
                materialdescriptionmapObj[rec.T2_revised_product_family__c] = relatedList;
                 ...
                 for(var j=0;j<uniqueMaterialDescptions.length;j++){
                     if(allData[i].materialDesc == uniqueMaterialDescptions[j]){
                         for(var k=0;k<currentKeysList.length;k++){
                             if(rec.T2_Month_Year__c == currentKeysList[k]){
