...
({
        ...
        loadModelAction.setCallback(this, function (response) {
            ...
            if (status === 'SUCCESS') {
               ...
               /*added by satya*/
                ...
                helper.insertLastActivityRec(component,event, helper)
            } else {
    ...
    insertLastActivityRec:function (component, helper){
        let recordId = component.get("v.recordId");
        ...
        action.setParams({
         ...
         action.setCallback(this, function (response) {
            var status = response.getState();
            if (status === 'SUCCESS') {
                console.log('----insert success test--');
            }else{
             console.log('----error in bidlast activity---' + response.getError());
            }
         });
        ...
        action.setCallback(this, function(response) 
                           {
                               if(response.getState()=="SUCCESS"){
                                   ...
                                   var hoverMsg = 'This Bid Visit: '+resposneString;
                                   ...
                                   console.log('error in bidApproval test')
                               }
                           });
                             $A.enqueueAction(action);
	}
