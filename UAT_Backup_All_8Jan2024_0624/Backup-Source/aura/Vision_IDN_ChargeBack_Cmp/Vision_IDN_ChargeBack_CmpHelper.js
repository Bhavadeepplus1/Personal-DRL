({
    getRegion:function(component, event, helper){
        ...
        action7.setCallback(this,function(response){
            if(response.getState() == 'SUCCESS'){
                ...
                component.set("v.regionList",title7);  
    ...
    gethospitalName:function(component, event, helper){
        ...
        action.setCallback(this,function(response){
            if(response.getState() == 'SUCCESS'){
                ...
                console.log('total hospital count12==>'+component.get("v.options3").length);
                var hospitalCount = responseList.length;
				component.set("v.hospitalCount",hospitalCount)
                component.set("v.loadingMessage",'Fetching '+component.get("v.options3").length+' Hospitals. ');
	...
	getFullData : function(component, event, helper) {
        ...
        var offsetVal = 0;
		var action = component.get("c.getDatafromLastLevBatch");
        action.setParams({
         ...
         action.setCallback(this,function(response){
             	 console.log('Status==>'+response.getState() );
                            if(response.getState() == 'SUCCESS'){
             					var responseList = response.getReturnValue();
                                console.log('response length current==>'+JSON.stringify(responseList)); 
                               ...
                               // var hospitalCount = responseList.length;
                                ...
                                var sixthColumnKey = '';
                                var categoryList =[];
                                //this.testmethod(component, event, helper,responseList,0)
                                for(var i=0;i<responseList.length;i++){
                                console.log('Success idnchargebackList==>'+ JSON.stringify((responseList[i])));
                                    hospitalList.push(responseList[i].hospitalName);
                                      //component.set("v.loadingMessage",'Fetching Hospitals. '+i+' out of '+responseList.length);
                               ...
                               // component.set("v.hospitalCount",hospitalCount)
                                ...
                                /*end for keys*/
                                var keysList = [];
                                keysList.push(firstColumnKey);
                                keysList.push(secondColumnKey);
                                keysList.push(thirdColumnKey);
                                 keysList.push(fourthColumnKey);
                                keysList.push(fifthColumnKey);
                                keysList.push(sixthColumnKey);
                                console.log('keysList==>'+keysList);
                                component.set("v.keysList",keysList);
                               ...
                               var testREc = component.get("v.chargebackList");
                                ...
                                component.set("v.allchargebackList",finaList);
                                var count = 0;
                               var initialRecList =finaList.slice(count , 50)
                               component.set("v.count",count+50);
                                component.set("v.chargebackList",initialRecList);
    ...
    testmethod :function(component, event, helper,responseList,hospitalCount){
        //count = count+1;
        var countList =[];
        component.set("v.loadingMessage",'hospitalCount');
        countList.push(component.get("v.loadingMessage"))
        component.set("v.countList",'countList');
        console.log('loadingMessage==>'+component.get("v.loadingMessage"));
        if(responseList.length == hospitalCount+1)
                    helper.finalRecordsReceived(component, helper, helper);
        			
                else{
                    //console.log('hospitalCount==>'+hospitalCount);
                    helper.testmethod(component,event, helper, responseList,hospitalCount+1);
                	
                }                                   
        
        
       // alert('what is the messg'+component.get("v.loadingMessage"))
        //console.log('what is the messg'+component.get("v.loadingMessage"));
        
    },
    finalRecordsReceived :function(component, event, helper){
       // component.set("v.loadingMessage",'');
        component.set("v.isSpinnerLoad", false); 
    },
  ...
  convertArrayOfObjectsToCSV : function(component,objectRecords){
      ...
      chargebackList.forEach(function(eachRec){
          var relatedList = eachRec.idnchargebackList;
          var totalsalesVal=0;
          var totalunitsVal=0;
          relatedList.forEach(function(chargeBackRec){
              var salesPrice = (chargeBackRec.Contract_Sales_Indirect_Price__c != null && chargeBackRec.Contract_Sales_Indirect_Price__c != undefined) ? chargeBackRec.Contract_Sales_Indirect_Price__c : 0;
              var units = (chargeBackRec.contract_units_paid_qty__c != null && chargeBackRec.contract_units_paid_qty__c != undefined) ? chargeBackRec.contract_units_paid_qty__c : 0;
              totalsalesVal= totalsalesVal + salesPrice;
              totalunitsVal=totalunitsVal+units;
          });
          var avgsalesVal= Math.round(((totalsalesVal/6) + Number.EPSILON) * 100) / 100;//Math.round(totalVal/6);
           var avgunitsVal= Math.round(((totalunitsVal/6) + Number.EPSILON) * 100) / 100;//Math.round(totalVal/6);
          var mainWrap = {
              			  'totalSales' :totalsalesVal,
              			  'totalUnits' :totalunitsVal,
              			  'AverageSales':avgsalesVal,
              			  'AverageUnits':avgunitsVal,
                          'eachHsptl':eachRec
                         };
          listOfWrappers.push(mainWrap);
      ...
      console.log('testing result listOfWrappers--->'+JSON.stringify(listOfWrappers));
        ...
        var myMap = new Map();
        myMap.set("Hospital Name", "hospitalName");
        myMap.set("Child1", "Child1_Name__c");
      	myMap.set("Child2", "Child2_Name__c");
      	myMap.set("Product Family", "revised_product_family__c");
     	myMap.set("Product", "material_description__c");
      	myMap.set("contract", "Contact_Description__c");
      	myMap.set("Month-Year", "Month_Year__c");
      	myMap.set("Sales in Jan-23", "Contract_Sales_Indirect_Price__c1");
      	myMap.set("Units in Jan-23", "contract_units_paid_qty__c1");
      	myMap.set("Sales in Feb-23", "Contract_Sales_Indirect_Price__c2");
      	myMap.set("Units in Feb-23", "contract_units_paid_qty__c2");
      	myMap.set("Sales in Mar-23", "Contract_Sales_Indirect_Price__c3");
      	myMap.set("Units in Mar-23", "contract_units_paid_qty__c3");
        myMap.set("Sales in Apr-23", "Contract_Sales_Indirect_Price__c4");
      	myMap.set("Units in Apr-23", "contract_units_paid_qty__c4");
        myMap.set("Sales in May-23", "Contract_Sales_Indirect_Price__c5");
      	myMap.set("Units in May-23", "contract_units_paid_qty__c5");
      	myMap.set("Sales in Jun-23", "Contract_Sales_Indirect_Price__c6");
        myMap.set("Units in Jun-23", "contract_units_paid_qty__c6");
      	myMap.set("Avg of 6 Month(Sales)", "AverageSales");
      	myMap.set("Avg of 6 Month(Units)", "AverageUnits");
      	myMap.set("Total of 6 Month(Sales)", "totalSales");
      	myMap.set("Total of 6 Month(Units)", "totalUnits");
        csvStringResult += Array.from(myMap.keys()).join(columnDivider);
        csvStringResult += lineDivider;
        //new logic start 
      listOfWrappers.forEach(function(eachREc){
          var idnChargeBackList = eachREc.eachHsptl.idnchargebackList;
          for(var i=0; i < idnChargeBackList.length; i++){  
              counter = 0;
              for (let [key, value] of myMap) {
                  ...
                  if(value=='hospitalName'){
                      csvStringResult += '"'+ idnChargeBackList[i].Hospital_Name__c +'"';
                  }
                  else if(value =='Child1_Name__c'){
                      csvStringResult += '"'+ idnChargeBackList[i].Child1_Name__c +'"';
                  }
                      else if(value =='Child2_Name__c'){
                          csvStringResult += '"'+ idnChargeBackList[i].Child2_Name__c +'"';
                      }
                          else if(value =='revised_product_family__c'){
                              csvStringResult += '"'+ idnChargeBackList[i].revised_product_family__c +'"';
                          }
                              else if(value =='material_description__c'){
                                  csvStringResult += '"'+ idnChargeBackList[i].material_description__c +'"';
                              }
                                  else if(value =='Contact_Description__c'){
                                      csvStringResult += '"'+ idnChargeBackList[i].Contact_Description__c +'"';
                                  }
                                      else if(value =='Month_Year__c'){
                                          csvStringResult += '"'+ idnChargeBackList[i].Month_Year__c +'"';
                                      }
                                          else if(value == 'Contract_Sales_Indirect_Price__c1'){
                                              if(idnChargeBackList[i].Month_Year__c == 'Jan-23'){
                                                  console.log('contracts sales==>'+idnChargeBackList[i].Contract_Sales_Indirect_Price__c)
                                                  csvStringResult += '"'+ idnChargeBackList[i].Contract_Sales_Indirect_Price__c +'"';
                                                  console.log('in second')
                                              }
                                          }
                                              else if(value == 'Contract_Sales_Indirect_Price__c2'){
                                                  if(idnChargeBackList[i].Month_Year__c == 'Feb-23'){
                                                      csvStringResult += '"'+ idnChargeBackList[i].Contract_Sales_Indirect_Price__c +'"';
                                                  }
                                              }
                                                  else if(value == 'Contract_Sales_Indirect_Price__c3'){
                                                      if(idnChargeBackList[i].Month_Year__c == 'Mar-23'){
                                                          csvStringResult += '"'+ idnChargeBackList[i].Contract_Sales_Indirect_Price__c +'"';
                                                      }
                                                  }
                                                      else if(value == 'Contract_Sales_Indirect_Price__c4'){
                                                          if(idnChargeBackList[i].Month_Year__c == 'Apr-23'){
                                                              csvStringResult += '"'+ idnChargeBackList[i].Contract_Sales_Indirect_Price__c +'"';
                                                          }
                                                      }
                                                          else if(value == 'Contract_Sales_Indirect_Price__c5'){
                                                              if(idnChargeBackList[i].Month_Year__c == 'May-23'){
                                                                  csvStringResult += '"'+ idnChargeBackList[i].Contract_Sales_Indirect_Price__c +'"';
                                                              }
                                                          }
                                                              else if(value == 'Contract_Sales_Indirect_Price__c6'){
                                                                  if(idnChargeBackList[i].Month_Year__c == 'Jun-23'){
                                                                      csvStringResult += '"'+ idnChargeBackList[i].Contract_Sales_Indirect_Price__c +'"';
                                                                  }
                                                              }
                                                                  else if(value == 'contract_units_paid_qty__c1'){
                                                                      if(idnChargeBackList[i].Month_Year__c == 'Jan-23'){
                                                                          csvStringResult += '"'+ idnChargeBackList[i].contract_units_paid_qty__c +'"';
                                                                      }
                                                                  }
                                                                      else if(value == 'contract_units_paid_qty__c2'){
                                                                          if(idnChargeBackList[i].Month_Year__c == 'Feb-23'){
                                                                              csvStringResult += '"'+ idnChargeBackList[i].contract_units_paid_qty__c +'"';
                                                                          }
                                                                      }
                                                                          else if(value == 'contract_units_paid_qty__c3'){
                                                                              if(idnChargeBackList[i].Month_Year__c == 'Mar-23'){
                                                                                  csvStringResult += '"'+ idnChargeBackList[i].contract_units_paid_qty__c +'"';
                                                                              }
                                                                          }
                                                                              else if(value == 'contract_units_paid_qty__c4'){
                                                                                  if(idnChargeBackList[i].Month_Year__c == 'Apr-23'){
                                                                                      csvStringResult += '"'+ idnChargeBackList[i].contract_units_paid_qty__c +'"';
                                                                                  }
                                                                              }
                                                                                  else if(value == 'contract_units_paid_qty__c5'){
                                                                                      if(idnChargeBackList[i].Month_Year__c == 'May-23'){
                                                                                          csvStringResult += '"'+ idnChargeBackList[i].contract_units_paid_qty__c +'"';
                                                                                      }
                                                                                  }
                                                                                      else if(value == 'contract_units_paid_qty__c6'){
                                                                                          if(idnChargeBackList[i].Month_Year__c == 'Jun-23'){
                                                                                              csvStringResult += '"'+ idnChargeBackList[i].contract_units_paid_qty__c +'"';
                                                                                          }
                                                                                      }
                                                                                          else if(value == 'AverageSales'){
                                                                                              csvStringResult += '"'+ eachREc.AverageSales +'"';
                                                                                              
                                                                                          }
                                                                                              else if(value == 'AverageUnits'){
                                                                                                  csvStringResult += '"'+ eachREc.AverageUnits +'"';
                                                                                                  
                                                                                              }
                                                                                                  else if(value == 'totalSales'){
                                                                                                      csvStringResult += '"'+ eachREc.totalSales +'"';
                                                                                                      
                                                                                                  }
                                                                                                      else if(value == 'totalUnits'){
                                                                                                          csvStringResult += '"'+ eachREc.totalUnits +'"';
    ...
    getAllTotals : function(component,event,helper) {
       ...
       //component.set("v.isSpinnerLoad", true)
        action.setParams({
         ...
         action.setCallback(this,function(response){
              console.log("response state in get All totals"+response.getState());
             if(response.getState() == 'SUCCESS'){
                 	var totalsResponse = response.getReturnValue();
                 console.log("totalsResponse main???"+JSON.stringify(totalsResponse));
                 console.log("sales price main ???==>"+Math.round(totalsResponse[0].salesPrice));
                 console.log("units main???==>"+totalsResponse[0].units);
         ...
         action.setCallback(this,function(response){
             if(response.getState() == 'SUCCESS'){
                ...
                for (var i=0; i<responseList.length; i++) {
                 ...
                 console.log('Error'+JSON.stringify(response.getError())); 
                 component.set("v.contratcsList",'');
             }
         })
          $A.enqueueAction(action);
     },
     getHosptalsData:function(component, pageNumber, pageSize){
         var action = component.get("c.getHosptalsData");
        action.setParams({
            "salesTerritory":component.get("v.selectedUserId"),
        ...
        action.setCallback(this, function(result) {
            ...
            if (component.isValid() && state === "SUCCESS"){
                var responseList = result.getReturnValue();
                                ...
                                var sixthColumnKey = '';
                                var categoryList =[];
                                for(var i=0;i<50;i++){//responseList.length
                                // console.log('Success idnchargebackList==>'+ JSON.stringify((responseList[i])));
                                  // console.log('Success currentSixMonthKeys==>'+responseList[i].currentSixMonthKeys[0]);
                                   ...
                                   responseList[i].idnchargebackList.forEach(function(rec){
                                      //userRegion = rec.Region__c;
                                    	categoryList.push(rec.Category__c);
                                       if(rec.H1_Month_Year__c == responseList[i].currentSixMonthKeys[5]){//oct-22
                                          console.log('H1_contract_units_paid_qty__c==>'+ rec.H1_contract_units_paid_qty__c);
                                           firstColSalesSummary += Math.round(rec.H1_Contract_Sales_Indirect_Price__c);
                                           firstColUnitsSummary +=  Math.round(rec.H1_contract_units_paid_qty__c);
                                       }
                                       if(rec.H1_Month_Year__c == responseList[i].currentSixMonthKeys[4]){
                                           secColSalesSummary += Math.round(rec.H1_Contract_Sales_Indirect_Price__c);
                                           secColUnitsSummary +=  Math.round(rec.H1_contract_units_paid_qty__c);
                                       }
                                       if(rec.H1_Month_Year__c == responseList[i].currentSixMonthKeys[3]){
                                           thirdColSalesSummary += Math.round(rec.H1_Contract_Sales_Indirect_Price__c);
                                           thirdColUnitsSummary +=  Math.round(rec.H1_contract_units_paid_qty__c);
                                       }
                                       if(rec.H1_Month_Year__c == responseList[i].currentSixMonthKeys[2]){
                                           fourthColSalesSummary += Math.round(rec.H1_Contract_Sales_Indirect_Price__c);
                                           fourthColUnitsSummary +=  Math.round(rec.H1_contract_units_paid_qty__c);
                                       }
                                       if(rec.H1_Month_Year__c == responseList[i].currentSixMonthKeys[1]){
                                           fifthColSalesSummary += Math.round(rec.H1_Contract_Sales_Indirect_Price__c);
                                           fifthColUnitsSummary +=  Math.round(rec.H1_contract_units_paid_qty__c);
                                       }
                                       if(rec.H1_Month_Year__c == responseList[i].currentSixMonthKeys[0]){
                                           sixthColSalesSummary += Math.round(rec.H1_Contract_Sales_Indirect_Price__c);
                                           sixthColUnitsSummary +=  Math.round(rec.H1_contract_units_paid_qty__c);
                                       }
                                   });
                               ...
                               var testREc = component.get("v.chargebackList");
                                ...
                                testREc.forEach(function(eachRec){
                                    ...
                                    var totalVal=0;
                                    relatedList.forEach(function(chargeBackRec){
                                        var salesPrice = (chargeBackRec.H1_Contract_Sales_Indirect_Price__c != null && chargeBackRec.H1_Contract_Sales_Indirect_Price__c != undefined) ? chargeBackRec.H1_Contract_Sales_Indirect_Price__c : 0;
                                        totalVal= totalVal + salesPrice;
                                ...
                                /*end Sorting for AVG of Sales*/
                                  ...
                                  component.set("v.isSpinnerLoad", false);
                component.set("v.PageNumber", responseList.pageNumber);
                component.set("v.TotalRecords", responseList.totalRecords);
                component.set("v.RecordStart", responseList.recordStart);
                component.set("v.RecordEnd", responseList.recordEnd);
                component.set("v.TotalPages", Math.ceil(responseList.totalRecords / pageSize));
            }
        });
        $A.enqueueAction(action);
