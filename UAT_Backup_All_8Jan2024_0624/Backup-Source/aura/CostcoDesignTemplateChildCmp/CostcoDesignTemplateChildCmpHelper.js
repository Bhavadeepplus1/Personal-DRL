    ...
    getCalculations: function (component, event, helper, currentValue, fieldName, isGuidancePrice, isMarketingPrice, isSalesPrice) {
        ...
        if ((WAC != 0 && WAC != undefined) && (WAC < proposedPriceSales || WAC < proposedPriceMarketing) && !isGuidancePrice && (isMarketingPrice || isSalesPrice)) {
            ...
            toastEvent.setParams({
                "type": "Error",
                "title": "Error",
                "message": 'Indirect Net can not be greater than WAC ($' + LineItem.Phoenix_WAC__c + ')'
            });
            toastEvent.fire();
            component.set("v.singleRec.Phoenix_Wholesaler_Diff_Price_Indirect__c", null);
        }
         /*   else if(proposedPriceMarketing<diffPriceIndirect && bidType!='Volume Review Only'){
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    "type":"Error",
                    "title": "Error",
                    "message": 'Indirect Price to be loaded @ABC & McKesson can not be less than Indirect Net'
                });
                toastEvent.fire();
                component.set("v.singleRec.Phoenix_ProposedContractBidPriceMktng__c", null);
            }*/
