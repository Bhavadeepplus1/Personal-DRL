({
    ...
    getItemsAfterFilter : function(component, event, helper){
        ...
        console.log('showRelevant  --> '+component.get("v.showRelevant"));
    ...
    getOnlyItems : function(component, event, helper){
        var accId = '';
        if(component.get("v.recordId")!= undefined)
            accId = component.get("v.recordId");
        else if(component.get("v.accId") != undefined)
    ...
    finalRecordsReceived : function(component, helper, wrapObj){
        ...
        if(gcpOptyTrcList.length>0){
            ...
            gcpOptyTrcList.forEach(function(gcpObj){
                ...
                if(gcpObj.Phoenix_Customer_Opp_TPT__c != undefined && !gcpObj.Vision_isZeroOptyVal__c)
                    custActOppTpt = custActOppTpt + parseInt(gcpObj.Phoenix_Customer_Opp_TPT__c);
                ...
                if(wrapObj.isErrorFromServer){
                    ...
                    component.set('v.isSpinnerLoad', false);
                        ...
                        if(gcpOptyTrcList.length>0){
                            ...
                            gcpOptyTrcList.forEach(function(gcpObj){
                                if(gcpObj.Phoenix_Est_Acct_Total_Vol_EU__c != undefined)
                                    actTotal = actTotal + parseInt(gcpObj.Phoenix_Est_Acct_Total_Vol_EU__c);
                                if(gcpObj.Phoenix_Est_Acct_Share_of_Wallet__c != undefined)
                                    shareOfWallet = shareOfWallet + parseInt(gcpObj.Phoenix_Est_Acct_Share_of_Wallet__c);
                                if(gcpObj.Phoenix_Drl_Act_Volume_Eu__c != undefined)
                                    drlAccVol = drlAccVol + parseInt(gcpObj.Phoenix_Drl_Act_Volume_Eu__c);
                                if(gcpObj.Phoenix_Drl_Act_Sales__c != undefined)
                                    drlAccSales = drlAccSales + parseInt(gcpObj.Phoenix_Drl_Act_Sales__c);
                                
                                if(gcpObj.Phoenix_Drl_Act_Price__c != undefined)
                                    drlAccPrice = drlAccPrice + parseInt(gcpObj.Phoenix_Drl_Act_Price__c);
                                if(gcpObj.Phoenix_DRL_Actual_TPT__c != undefined)
                                    drlAccTpt = drlAccTpt + parseInt(gcpObj.Phoenix_DRL_Actual_TPT__c);
                                if(gcpObj.Phoenix_Total_Market_Vol_MAT_EU__c != undefined)
                                    totalMktVol = totalMktVol + parseInt(gcpObj.Phoenix_Total_Market_Vol_MAT_EU__c);
                                if(gcpObj.Phoenix_Total_Market_Sales__c != undefined)
                                    totalMktShares = totalMktShares + parseInt(gcpObj.Phoenix_Total_Market_Sales__c);
                                
                                if(gcpObj.Phoenix_Cust_Act_Volume_Mat_Eu__c != undefined)
                                    custActVol = custActVol + parseInt(gcpObj.Phoenix_Cust_Act_Volume_Mat_Eu__c);
                                if(gcpObj.Phoenix_Cust_Act_Mkt_Sh__c != undefined)
                                    custActMktShare = custActMktShare + parseInt(gcpObj.Phoenix_Cust_Act_Mkt_Sh__c);
                                if(gcpObj.Phoenix_Customer_Opp_TPT__c != undefined)
                                    custActOppTpt = custActOppTpt + parseInt(gcpObj.Phoenix_Customer_Opp_TPT__c);
                                console.log('custActOppTpt --> '+custActOppTpt);
                                if(continueLoop){
                                    monthRange = gcpObj.Phoenix_Ims_Qtr_Month_Range__c != undefined && gcpObj.Phoenix_Ims_Qtr_Month_Range__c != null ? gcpObj.Phoenix_Ims_Qtr_Month_Range__c : '';
...
if(offsetVal+1 == totalList.length){
    ...
    getStatusCount: function(component, event, helper, offsetVal){
        ...
        if(gcpList.length > 0){
            gcpList.forEach(function(item){
                item.Vision_isRelevance__c = item.Vision_isRelevance__c != undefined ? item.Vision_isRelevance__c : true;
            ...
            toastEvent.setParams({
                "title": "ERROR",
                "message": 'No products are selected to Review.',
                "type":"ERROR",
                "mode":"dismissible"
            });
            toastEvent.fire();
    ...
    getTheCount : function(component, event, helper, gcpList, offsetVal){
        ...
        if(!processingItem.Vision_isZeroOptyVal__c && processingItem.Vision_isRelevance__c){
            ...
            action.setCallback(this, function(response) {
                ...
                if (state === "SUCCESS"){
                    ...
                    if(component.get("v.accObj").Vision_is_SRx_Account__c){
                        ...
                        reviewProductsObj.prodStatusBothCons = reviewProductsObj.prodStatusBothCons + wrapperObj.prodStatusBothCons;
                        ...
                        reviewProductsObj.bothConInBidCount = reviewProductsObj.bothConInBidCount + wrapperObj.bothConInBidCount;
                    ...
                    reviewProductsObj.bothConDollar = reviewProductsObj.bothConDollar + wrapperObj.bothConDollar;
                    ...
                    if(wrapperObj.bothConCount>0){
                        reviewProductsObj.bothConCount = reviewProductsObj.bothConCount + wrapperObj.bothConCount;
                    }
    ...
    getNDCListForPieChartController : function(component, event, helper){
        ...
        action.setParams({accId:component.get("v.recordId"),
                          prodStatus:component.get("v.selectedTabInPieChart"),
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS"){
                ...
                listItems.forEach(function(obj){
                    if(obj.Vision_Estimate_Account_Total_Vol_EU__c != undefined){
                        summaryObj.totalsOfEa += parseInt(obj.Vision_Estimate_Account_Total_Vol_EU__c);
                        if(obj.Vision_Product__r.Phoenix_Pkg_Size__c != undefined)
                            summaryObj.totalsOfEu += parseInt(obj.Vision_Estimate_Account_Total_Vol_EU__c)*parseInt(obj.Vision_Product__r.Phoenix_Pkg_Size__c);
                    }
                    if(obj.GCP_Cust_Act_Volume_Mat_Eu__c != undefined)
                        summaryObj.custVol += parseInt(obj.GCP_Cust_Act_Volume_Mat_Eu__c);
                    if(obj.Vision_Cust_Act_Sales_Mat__c != undefined)
                        summaryObj.custSales += parseInt(obj.Vision_Cust_Act_Sales_Mat__c);
                });
                component.set("v.summaryObj",summaryObj);
                component.set("v.ndcListFromPieChart",listItems);
            }
            else{
                component.set("v.isSpinnerLoad",false);
                console.log('ERROR from updateNdcLineItem --> '+JSON.stringify(response.getError()));
    ...
    convertArrayOfObjectsToCSV : function(component, event, helper, ndcListFromPieChart){
        var accObj = component.get("v.accObj");
        //var ndcListFromPieChart = component.get("v.ndcListFromPieChart");
        // declare variables
        ...
        csvStringResult = '';
        var myMap = new Map();
        myMap.set("Product Name", "Vision_Product__r.Name");
        myMap.set("NDC", "Vision_Product__r.Phoenix_NDC_11__c");
        if(accObj.Vision_is_SRx_Account__c){
            myMap.set("Total DRL Market Actual Vol(EU)", "Vision_DRL_Act_Volume_Eu__c");
            myMap.set("Total DRL Market Actual Sales", "Vision_Drl_Act_Sales__c");
            myMap.set("Total DRL Market Actual Price($/EU)", "Vision_Drl_Act_Price__c");
            myMap.set("Total DRL Market Actual TPT", "Vision_DRL_Act_Tpt__c");
            myMap.set("Total DRL Market Actual TPT %", "Vision_Drl_Act_Tpt_Pct__c");
        }
        if(!accObj.Vision_is_SRx_Account__c){
            myMap.set("Estimate Vol(EA)", "Vision_Estimate_Account_Total_Vol_EU__c");
            myMap.set("Estimate Vol(EU)", "estimateVolEu");
        }
        myMap.set("DRL Customer Actual Vol", "GCP_Cust_Act_Volume_Mat_Eu__c");  
        myMap.set("DRL Customer Actual Sales", "Vision_Cust_Act_Sales_Mat__c");  
        myMap.set("DRL Customer Avg Price", "Vision_Cust_Act_Sales_Mat__c/GCP_Cust_Act_Volume_Mat_Eu__c");
        if(accObj.Vision_is_SRx_Account__c){
            myMap.set("Opportunity Share %", "Vision_SRx_Opty_Share_per__c");
            myMap.set("Opportunity Share Vol", "Vision_SRx_Opportunity_Dollar__c");
        }
        myMap.set("Probability", "Vision_Probability__c");
        myMap.set("Status", "statusItems");
        csvStringResult += Array.from(myMap.keys()).join(columnDivider);
        csvStringResult += lineDivider;
        //new logic start 
        for(var i=0; i < ndcListFromPieChart.length; i++){  
            counter = 0;
            for (let [key, value] of myMap) {
                if(counter > 0)
                    csvStringResult += columnDivider; 
                if(value=='Vision_Product__r.Name')
                    csvStringResult += '"'+ ndcListFromPieChart[i]["Vision_Product__r"]["Name"]+'"';
                else if(value=='Vision_Product__r.Phoenix_NDC_11__c')
                    csvStringResult += '"'+ ndcListFromPieChart[i]["Vision_Product__r"]["Phoenix_NDC_11__c"]+'"';
                    else if(value == 'Vision_Estimate_Account_Total_Vol_EU__c'){
                        var totalVolEu=ndcListFromPieChart[i]['Vision_Estimate_Account_Total_Vol_EU__c'];
                        if(totalVolEu != null){
                            var rounded_leVolEA = Math.round(totalVolEu*100)/100;
                            csvStringResult += '"'+ rounded_leVolEA +'"';
                        }
                        else
                            csvStringResult += '"'+'0'+'"';
                    }
                        else if(value == 'Vision_Cust_Act_Sales_Mat__c'){
                            var latest_estimateSal=ndcListFromPieChart[i]['Vision_Cust_Act_Sales_Mat__c'];
                            if(latest_estimateSal != null){
                                var rounded_leSal=Math.round(latest_estimateSal*100)/100;
                                csvStringResult += '"'+ rounded_leSal +'"';
                            }
                            else
                                csvStringResult += '"'+'0'+'"';
                        }
                            else if(value == 'GCP_Cust_Act_Volume_Mat_Eu__c'){
                                var latest_estimateVol=ndcListFromPieChart[i]['GCP_Cust_Act_Volume_Mat_Eu__c'];
                                if(latest_estimateVol != null){
                                    var rounded_leVol=Math.round(latest_estimateVol*100)/100;
                                    csvStringResult += '"'+ rounded_leVol +'"';
                                }
                                else
                                    csvStringResult += '"'+'0'+'"';
                            }
                                else if(value == 'estimateVolEu'){
                                    var EaVol = ndcListFromPieChart[i]['Vision_Estimate_Account_Total_Vol_EU__c'] != null ? ndcListFromPieChart[i]['Vision_Estimate_Account_Total_Vol_EU__c'] : 0;
                                    var PkgSize = ndcListFromPieChart[i]['Vision_Product__r']['Phoenix_Pkg_Size__c'] != null ? ndcListFromPieChart[i]['Vision_Product__r']['Phoenix_Pkg_Size__c'] : 0;
                                    var estVol = EaVol*PkgSize;
                                    if(estVol != null && estVol > 0){
                                        var rounded_leVolEu=Math.round(estVol*100)/100;
                                        csvStringResult += '"'+ rounded_leVolEu +'"';
                                    }
                                    else
                                        csvStringResult += '"'+'0'+'"';
                                }
                                    else if(value == 'Vision_Cust_Act_Sales_Mat__c/GCP_Cust_Act_Volume_Mat_Eu__c'){
                                        var latest_estimateAvg=ndcListFromPieChart[i]['Vision_Cust_Act_Sales_Mat__c']/ndcListFromPieChart[i]['GCP_Cust_Act_Volume_Mat_Eu__c'];
                                        if(latest_estimateAvg != null && latest_estimateAvg > 0){
                                            var rounded_leAvg=Math.round(latest_estimateAvg*100)/100;
                                            csvStringResult += '"'+ rounded_leAvg +'"';
                                        }
                                        else
                                            csvStringResult += '"'+'0'+'"';
                                    }
                                        else if(value == 'Vision_SRx_Opty_Share_per__c'){
                                            var latest_estimateShare=ndcListFromPieChart[i]['Vision_SRx_Opty_Share_per__c'];
                                            if(latest_estimateShare != null){
                                                var rounded_leShare=Math.round(latest_estimateShare*100)/100 + '%';
                                                csvStringResult += '"'+ rounded_leShare +'"';
                                            }
                                            else
                                                csvStringResult += '0';
                                        }
                                            else if(value == 'Vision_SRx_Opportunity_Dollar__c'){
                                                var latest_estimateDol=ndcListFromPieChart[i]['Vision_SRx_Opportunity_Dollar__c'];
                                                if(latest_estimateDol != null){
                                                    var rounded_leDol=Math.round(latest_estimateDol);
                                                    csvStringResult += '"'+ rounded_leDol +'"';
                                                }
                                                else
                                                    csvStringResult += '0';
                                            }
                                                else if(value == 'statusItems'){
                                                    if(!ndcListFromPieChart[i]['Vision_Product_Constraint__c'].includes('None') && ndcListFromPieChart[i]['Vision_Product_Constraint__c'] != undefined)
                                                        csvStringResult += '"'+ ndcListFromPieChart[i]['Vision_Product_Constraint__c'] +'"';
                                                    else if(ndcListFromPieChart[i]['Vision_Product_Status__c'] != undefined){
                                                        csvStringResult += '"'+ ndcListFromPieChart[i]['Vision_Product_Status__c'] +'"';
                                                    }
                                                        else
                                                            csvStringResult += '0';
                                                }
                                                else if(ndcListFromPieChart[i][value]==undefined)
                                                    csvStringResult += '"'+''+'"';
                                                    else
                                                        csvStringResult += '"'+ ndcListFromPieChart[i][value]+'"';
