({
	 initCharts: function(component, event, helper){
        ...
        materialDescRelatedList.forEach(function(rec){
            ...
            if(contractdescriptionmapObj.hasOwnProperty(rec.T2_material_description__c)){
                console.log('inside if==>'+rec.T2_material_description__c);
                var relatedList = contractdescriptionmapObj[rec.T2_material_description__c];
                relatedList.push(rec);
                contractdescriptionmapObj[rec.T2_material_description__c] = relatedList;
