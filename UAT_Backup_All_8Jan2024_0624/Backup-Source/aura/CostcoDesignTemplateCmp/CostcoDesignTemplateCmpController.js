...
({
    ...
    submitFor: function (component, event, helper) {
        ...
        var lineItemsList = component.get("v.BidLineItemListAll");
        var isBidNotValid;
        ...
        for (var i = 0; i < lineItemsList.length; i++) {
              var deadnet = lineItemsList[i].Phoenix_Indirect_Dead_Net__c;
             var lowestPrice = lineItemsList[i].Phoenix_Lowest_Price_SKU__c;
             console.log('deadnet-->'+deadnet);
             console.log('lowestPrice-->'+lowestPrice);
            if(deadnet < lowestPrice){
             var isBidNotValid = "You cannot submit this Bid as Dead Net is Less than Lowest/SKU" ;
        	} 
         }
        }*/
        if (isBidNotValid) {
            ...
            action.setCallback(this, function (response) {
                if (response.getState() == "SUCCESS") {
                    ...
                    for (var i = 0; i < bidlines.length; i++) {
                        console.log(JSON.stringify(bidlines[i].Phoenix_ProposedContractBidPriceMktng__c))
                        if ((templateType == 'Costco' || templateType == 'Walgreens' || templateType == 'ABC Progen') && bidtype != 'Price Change' && bidtype != 'Customer Rebate Change' && bidtype != 'Sales Out Rebate' && bidlines[i].Phoenix_Proposed_Indirect_Selling_Unit__c == null) {
                        ...
                        /*else if(bidtype!='Volume Review Only' && component.get("v.isMarketingApprovePerson")==undefined && bidlines[i].Phoenix_ProposedContract_Bid_Price_Sales__c==null){
                            ...
                            qtyError = 'Please Enter Indirect Price to be loaded @ABC & McKesson ' + bidlines[i].Phoenix_Product__r.Name;
                            console.log('test markt price-->'+bidlines[i].Phoenix_ProposedContractBidPriceMktng__c);
                            ...
                            qtyError = 'Please Enter Indirect Net Price '  + bidlines[i].Phoenix_Product__r.Name;
                            console.log('Indirect Net Price');
                            break;
                        }
                            else if(component.get("v.isMarketingApprovePerson")==component.get("v.loggedInUserName") && (component.get("v.loggedInUserName") == bidlines[i].Phoenix_Product_Director1__c || (delegatedUser != null && delegatedUser.includes(bidlines[i].Phoenix_Product_Director1__c)))
                                    && bidlines[i].Phoenix_ProposedContractBidPriceMktng__c < bidlines[i].Phoenix_Wholesaler_Diff_Price_Indirect__c && bidtype!='Volume Review Only'){
                                qtyError='Indirect Price to be loaded @ABC & McKesson can not be less than Indirect net ' + bidlines[i].Phoenix_Product__r.Name;
                    ...
                    for (var i = 0; i < bidlines.length; i++) {
                          // for ovveride units
                                        if (bidtype != 'New Product Launch' && (bidlines[i].Phoenix_Override_Current_Indirect_Units__c == null && (bidlines[i].Phoenix_Current_Indirect_Selling_Unit__c == 0 || bidlines[i].Phoenix_Current_Indirect_Selling_Unit__c == undefined || bidlines[i].Phoenix_Current_Indirect_Selling_Unit__c == null))){
                                            console.log('isOverrideUnitsavl-----'+bidlines[i].Phoenix_Override_Current_Indirect_Units__c);
                    ...
                    console.log('isOverrideUnitsavl-----'+isOverrideUnitsavl);
                       var modalMessage = isOverrideUnitsavl ? 'The 3 Months Annualized Current Indirect Units is 0. Are you sure you want to submit this bid for approval?' : 'Are you sure you want to submit this bid for approval?';
    ...
    initRecords: function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            if (response.getState() == "SUCCESS") {
                ...
                component.set("v.BidLineItemOldListAll", lineItemsList);
