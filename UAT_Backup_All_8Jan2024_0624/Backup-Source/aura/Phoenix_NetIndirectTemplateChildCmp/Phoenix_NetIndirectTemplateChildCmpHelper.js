({
    ...
    getCalculations : function(component,event,helper, currentValue, fieldName,isGuidancePrice,isMarketingPrice,isSalesPrice,isPropIndirectmarketing){  
        ...
        if((WAC != 0 && WAC != undefined) && (WAC < proposedPriceSales || WAC < proposedPriceMarketing) && !isGuidancePrice){
            ...
            toastEvent.setParams({
                "type":"Error",
                "title": "Error",
                "message": 'Proposed Indirect Price can not be greater than WAC ($'+LineItem.Phoenix_WAC__c+')'
            });
            toastEvent.fire();
            ...
            toastEvent.setParams({
                "type":"Error",
                "title": "Error",
                "message": 'Proposed Indirect Price can not be less than Proposed Direct Price'
            });
            toastEvent.fire();
            component.set("v.singleRec.Phoenix_Wholesaler_Diff_Price_Indirect__c", null);
        }
       /* if(isPropIndirectmarketing){
            if(LineItem.Phoenix_Customer_Dead_Net1__c > diffPriceIndirect){
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    "type":"Error",
                    "title": "Error",
                    "message": 'Proposed Indirect Price can not be less than Net Indirect Price'
                });
                toastEvent.fire();
                component.set("v.singleRec.Phoenix_Wholesaler_Diff_Price_Indirect__c", null);
            }
