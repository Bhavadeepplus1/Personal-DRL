({
        ...
        action.setCallback(this, function(response) 
                           {
                               if(response.getState()=="SUCCESS"){
                                   ...
                                   component.set("v.proNetSaleDir",responseValue[0].netSaleDir);
                                   component.set("v.proNetSaleInd",responseValue[0].netSaleIndir);
                                   component.set("v.proTPTDir",responseValue[0].tptDir);
                                   component.set("v.proTPTInd",responseValue[0].tptIndir);
                                   ...
                                   component.set("v.overRideDirectUnits",responseValue[0].OverrideDirUnits);
                                   ...
                                   component.set("v.FinalDirUnits",responseValue[0].fdsu);
                                   component.set("v.FinalIndirectUnits",responseValue[0].fisu);
                                   component.set("v.finalTotalUnis",responseValue[0].ftsu);
                                   component.set("v.FinalExntendUnits",responseValue[0].finalAnulUnits);
    ...
    convertArrayOfObjectsToCSV : function(component,objectRecords,template,bidType,isReBid){
        ...
        if(bidType == "RFP Bids"|| bidType == "New Product Launch" || bidType == "Product Addition" || bidType == "Volume Review Only"){
            ...
            myMap.set("Direct Rx Outreach (Current + Increment)", "Phoenix_Direct_Rx_Outreach__c");
        ...
        action.setCallback(this, function(response) 
                           {
                               if (response.getState() === "SUCCESS") {
                                   ...
                                   for(var i=0;i<resultlength;i++){
                                       if(DirectCons.length>0 && bidtype!='Price Change' && bidtype!='Customer Rebate Change' && bidtype !='Sales Out Rebate' && (ResultData[i].Phoenix_Direct_ESI__c==null  && ResultData[i].Phoenix_Direct_Kroger__c==null  && ResultData[i].Phoenix_Direct_Rx_Outreach__c==null  && ResultData[i].Phoenix_Direct_Supervalu__c==null && ResultData[i].Phoenix_Direct_Cigna__c==null && ResultData[i].Phoenix_Direct_Cordant__c==null && ResultData[i].Phoenix_Direct_Accerodo__c==null &&  ResultData[i].Phoenix_Others_Direct__c==null )){
                                           qtyError='Please Enter Others Direct Selling Units for '+ResultData[i].Phoenix_Product__r.Name;
                                           console.log('in direct');
                                           break;
                                       }
                                       if(indirectCons.length>0 && bidtype!='Price Change' && bidtype!='Customer Rebate Change' && bidtype !='Sales Out Rebate' && (ResultData[i].Phoenix_Indirect_ESI__c==null  && ResultData[i].Phoenix_Indirect_Kroger__c==null  && ResultData[i].Phoenix_Indirect_Rx_Outreach__c==null  && ResultData[i].Phoenix_Indirect_Supervalu__c==null && ResultData[i].Phoenix_Indirect_Cigna__c==null && ResultData[i].Phoenix_Indirect_Cordant__c==null && ResultData[i].Phoenix_Indirect_Accerodo__c==null &&  ResultData[i].Phoenix_Others_Indirect__c==null )){
                                       ...
                                       if(component.get("v.BidAprrovalStatus")!='Draft' && (ResultData[i].Phoenix_Product_Director__c==component.get("v.loggedInUserName") || (delegatedUser != null && delegatedUser.includes(ResultData[i].Phoenix_Product_Director__c))) && ResultData[i].Phoenix_Wholesaler_Diff_Price_Indirect__c==null && bidtype != 'Customer Rebate Change' && bidtype != 'Volume Review Only' && bidtype != 'Sales Out Rebate' ){	
                                           if(ResultData[i].Phoenix_Marketing_Approval__c !='Not Approved'){
                                               qtyError='Please Enter Wholesaler Differnce Price (Indirect)';	
                                               console.log('marketing Priceee');	
                                               break;	  
                                           } else{
                                               break;
                                           }
                                   ...
                                   for(var i=0;i<resultlength;i++){
                                        // for ovveride units
                                           if(component.get("v.BidAprrovalStatus")!='Draft' && ResultData[i].Phoenix_Product_Director__c==component.get("v.loggedInUserName") && bidtype !='New Product Launch' && ResultData[i].Phoenix_Override_Current_Indirect_Units__c==null && (ResultData[i].Phoenix_Current_Indirect_Selling_Unit__c==null || ResultData[i].Phoenix_Current_Indirect_Selling_Unit__c==undefined || ResultData[i].Phoenix_Current_Indirect_Selling_Unit__c==0)){                                             
                                                   ...
                                                   else if(component.get("v.BidAprrovalStatus")!='Draft' && ResultData[i].Phoenix_Product_Director__c==component.get("v.loggedInUserName") && bidtype !='New Product Launch' && ResultData[i].Phoenix_Override_Current_Direct_Units__c==null && (ResultData[i].Phoenix_Current_Direct_Selling_Unit__c==null || ResultData[i].Phoenix_Current_Direct_Selling_Unit__c==undefined || ResultData[i].Phoenix_Current_Direct_Selling_Unit__c==0)){                                             
                                                       isOverrideUnitsavl =true;
                                                       break;
                                                   }
