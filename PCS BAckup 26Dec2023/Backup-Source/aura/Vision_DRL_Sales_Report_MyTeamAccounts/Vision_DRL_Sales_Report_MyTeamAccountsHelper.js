({
    getData: function(component, event, helper,selectedUserId,showInt){
        console.log('showInt--->'+showInt)
        console.log('userName in helper-->'+selectedUserId);
        console.log('in helper search text-->'+component.get("v.searchText"));
        ...
        action.setCallback(this, function(response) 
                           {
                               if(response.getState()=="SUCCESS"){
                                   ...
                                   var test=[];
                                   var userNames = response.alluserNamesUnserSameManager;
                                    var pickList =  [];
                                   var picklistValues = response.picklistValues;
                                   var i = 1;
                                   picklistValues.forEach(function(user){
                                       pickList.push({userId:''+user.Id,userName:''+user.Name,isSelected:(selectedUserId == undefined || selectedUserId == ''?( i==1 ? true : false ):( user.Id==selectedUserId?true:false))});
                                       i++;
                                   });
                                   ...
                                   for(var i=0;i<userNames.length;i++){
                                     test.push(userNames[i],userNames[i])  ;
                                   }
                                   console.log('testformat-->'+test);
                                   ...
                                   console.log('keyset myteam-->'+JSON.stringify(response.keySet))
                                    console.log('keySetPrev myteam-->'+JSON.stringify(response.keySetPrev))
                                    component.set("v.res",response.accWrapMap);
                                   /* if(response.finalPrevMapSummary != null){
                                   ...
                                   if(response.accWrapMap.length != 0){
                                       ...
                                       for (var key in response.finalPrevMapSummary){
                                           ...
                                           if(comparison != 'lastquartervspreviousquarter'){
                                               ...
                                               sumofSales += Math.round(response.finalPrevMapSummary[key]['prevsalesMonthlySummary']);
    ...
    buildData : function(component, event, helper, response) {
        ...
        console.log('len--'+component.get("v.responseList").length )
    ...
    getChartData: function(component, event, helper,showInt,recentLastMonth,previousLastMonth){
        component.set('v.loaded', true);
        console.log('showInt in getChartData'+showInt);
        ...
        console.log('selectedUserId in get chart data--->'+component.get("v.selectedUser"));
        var action = component.get("c.getDRLSalesRelatedList");
        action.setParams({
            selections: component.get("v.selections"),
            selectedComparison: component.get("v.comparison"),
            customerIds: component.get("v.customerIds"),
            selectedUserId : component.get("v.selectedUser"),
            showInt:showInt,
