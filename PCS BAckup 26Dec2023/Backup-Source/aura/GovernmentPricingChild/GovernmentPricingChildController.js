({    
    ...
    calcGoalSeek : function(component,event,helper){
        ...
        //for goal sekk calculation
        var GSeekDivident = component.get("v.singleRec.Phoenix_Direct_CD_Per_Unit__c") != null ? parseFloat(component.get("v.singleRec.Phoenix_Direct_CD_Per_Unit__c")) : 0;
        let requiredPricedeNumer;
        let currentcashterms=bidRecord.Phoenix_Indirect_CD_Per__c;
        ...
        let RDCPer=bidRecord.Phoenix_RDC_NLC__c;
        //if(RDCPer!=null){RDCPer=parseFloat(RDCPer/100);}
        RDCPer = RDCPer!=null ? parseFloat(RDCPer/100) : 0;
        let WACValue=bidRecord.Phoenix_WAC__c;
        //if(WACValue!=null){WACValue=parseFloat(WACValue);}
        WACValue = WACValue!=null ? parseFloat(WACValue) : 0;
        let RebatePer=bidRecord.Phoenix_Proposed_Current_Rebate__c;
        //if(RebatePer!=null){RebatePer=parseFloat(RebatePer/100);}
        RebatePer = RebatePer!=null ? parseFloat(RebatePer/100) : 0;
        let CMfeePer=bidRecord.Phoenix_Contract_Mngment_Fee_Wholesaler__c;
        //if(CMfeePer!=null){CMfeePer=parseFloat(CMfeePer/100);}
        CMfeePer = CMfeePer!=null ? parseFloat(CMfeePer/100) : 0;
        let Cal1=currentcashterms+RDCPer;
        let Cal2=Cal1*WACValue;
        let Cal3=CMfeePer+RebatePer;
        
        component.set("v.RequiredPrice",(requiredPriceNumer+Cal2)/(1-Cal3));
        ...
        /*var PICPrice = component.get("v.singleRec.Phoenix_Wholesaler_Diff_Price_Indirect__c");
            if(WAC<PICPrice){
				//onCISUChange(component, event,helper);
				component.set("v.errMsg","Marketing Price can not be greater than WAC");
                component.set("v.err",true);
    ...
    calcCustDeadNetGoalSeek:  function(component,event,helper){
        ...
        let cashDsicount=component.get("v.singleRec.Phoenix_Customer_Cash_Discount1__c") != null ? parseFloat(component.get("v.singleRec.Phoenix_Customer_Cash_Discount1__c")) : 0;
        console.log('cash discount--->'+component.get("v.singleRec.Phoenix_Customer_Cash_Discount1__c"))
        let iodPerUnit= component.get("v.singleRec.Phoenix_IOD_Per_Unit_in__c") != null ? parseFloat(component.get("v.singleRec.Phoenix_IOD_Per_Unit_in__c")) : 0;
        ...
        let adminfee = component.get("v.singleRec.Phoenix_Customer_Fee__c") != null ? parseFloat(component.get("v.singleRec.Phoenix_Customer_Fee__c")/100) : 0;
        console.log('cmFeePerc--->'+adminfee);
        let rebate=component.get("v.singleRec.Phoenix_Proposed_Current_Rebate__c") != null ? parseFloat(component.get("v.singleRec.Phoenix_Proposed_Current_Rebate__c")/100) : 0;
        //let rebateABC=parseFloat(component.get("v.singleRec.Phoenix_Customer_Rebates__c")/100);
        //console.log('rebateABC--->'+rebateABC);
        let gorupVipPerc=component.get("v.singleRec.Phoenix_Bid__r.Phoenix_Group_VIP__c") != null ? parseFloat(component.get("v.singleRec.Phoenix_Bid__r.Phoenix_Group_VIP__c")/100) : 0;
        let orderAnlFeePerc=component.get("v.singleRec.Phoenix_Customer_Order_Analytics_Fee__c") != null ? parseFloat(component.get("v.singleRec.Phoenix_Customer_Order_Analytics_Fee__c")/100) : 0;
        ...
        let loaclVIP=component.get("v.singleRec.Phoenix_Local_VIP_Per_Unit__c") != null ? parseFloat(component.get("v.singleRec.Phoenix_Local_VIP_Per_Unit__c")/100): 0;
        let otherloaclVIP=component.get("v.singleRec.Phoenix_Local_VIP__c") != null ? parseFloat(component.get("v.singleRec.Phoenix_Local_VIP__c")/100) : 0;
        console.log('Other loaclVIP--->'+otherloaclVIP);
        //var templateType =component.get('v.templateType')
        //parseFloat(component.get("v.singleRec.Phoenix_Contract_Mngment_Fee_Wholesaler__c"));
        let currentcashterms=(component.get("v.singleRec.Phoenix_Indirect_CD_Per__c"));
        //if(currentcashterms!=null){currentcashterms=parseFloat(currentcashterms/100);}
        currentcashterms = currentcashterms!=null ? parseFloat(currentcashterms/100) : 0;
        let RDCPer=(component.get("v.singleRec.Phoenix_RDC_NLC__c"));
        //if(RDCPer!=null){RDCPer=parseFloat(RDCPer/100);}
        RDCPer = RDCPer!=null ? parseFloat(RDCPer/100) : 0;
        let WACValue=(component.get("v.singleRec.Phoenix_WAC__c"));
       // if(WACValue!=null){WACValue=parseFloat(WACValue);}
        WACValue = WACValue!=null ? parseFloat(WACValue) : 0;
        let rebate1=parseFloat(component.get("v.singleRec.Phoenix_Proposed_Current_Rebate__c"));
        let currebate=parseFloat(component.get("v.singleRec.Phoenix_Current_Rebate__c"));
        currebate = Number.isFinite(currebate) ? currebate : 0;
        rebate1 = Number.isFinite(rebate1) ? rebate1 : 0;
        rebate1 = rebate1 > 0 ? rebate1 :currebate;
        let RebatePer=rebate1;
        //if(RebatePer!=null){RebatePer=parseFloat(RebatePer/100);}
        RebatePer = RebatePer!=null ? parseFloat(RebatePer/100) : 0;
        
        
        let CMfeePer=(component.get("v.singleRec.Phoenix_Contract_Mngment_Fee_Wholesaler__c"));
        //if(CMfeePer!=null){CMfeePer=parseFloat(CMfeePer/100);}
        CMfeePer = CMfeePer!=null ? parseFloat(CMfeePer/100) : 0;
        let Cal1=currentcashterms+RDCPer;
        let Cal2=Cal1*WACValue;
        ...
        component.set("v.RequiredPrice",(deadnetPrice+Cal2)/(1-Cal3));
        console.log((deadnetPrice+Cal2)/(1-Cal3) +'cal');
        component.set("v.expectCustmerDdeadnet",null);
        console.log("deadnet sai test --"+component.get("v.singleRec.Phoenix_Indirect_Dead_Net__c")	);
        ...
        if(WAC<proposedPriceMarketing){
            ...
            console.log("test price value--"+component.get("v.singleRec.Phoenix_Wholesaler_Diff_Price_Indirect__c")	);
    ...
    onCISUChange : function(component,event,helper){
        /*  var rebatepercalc =component.get("v.singleRec.Phoenix_Proposed_Current_Rebate__c");
            if(rebatepercalc != null){parseFloat(rebatepercalc/100);}
            var indirectPrice = component.get("v.singleRec.Phoenix_Wholesaler_Diff_Price_Indirect__c");
            if(indirectPrice != null){indirectPrice=indirectPrice;}
            
            var rebate = rebatepercalc * indirectPrice;
            component.set("v.GPRebate",rebate);
           */
            ...
            if( goalseek == true){
                ...
                var copyText=component.get("v.RequiredPrice");
                console.log("required pirce copyText-->"+copyText);
    ...
    //new methods for new product launch
