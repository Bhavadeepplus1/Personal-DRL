        ...
        action.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS") {
                ...
                for (var i = 0; i < resultlength; i++) {
                    if (bidType != 'Volume Review Only' && bidType != 'Sales Out Rebate' && component.get("v.BidAprrovalStatus") != 'Draft' && (ResultData[i].Phoenix_Product_Director__c == component.get("v.loggedInUserName") || (deligatedUserName != null && deligatedUserName.includes(ResultData[i].Phoenix_Product_Director1__c))) && ResultData[i].Phoenix_Wholesaler_Diff_Price_Indirect__c == null && ResultData[i].Phoenix_Marketing_Approval__c != 'Not Approved') {
