({
    doInit: function(component, event, helper){
        ...
        for(var i=(qList.length-1); i>=0; i--){
            var obj ={};
            var yearString;
            console.log('Quarter: '+qList[i]);
            console.log('Quarter Number: '+String(qList[i])[1]);
            ...
            console.log('foundQ4: '+foundQ4);
        }
        /*var currentYear = String(year);
        var list = [];var quarters = [];
        var previousYear = String(year-1);
        var obj1 = {};
        obj1.label = quarterStrings[quarterStrings.indexOf(currentQuarter)-1]+' FY'+previousYear.charAt(previousYear.length-2)+previousYear.charAt(previousYear.length-1);
        obj1.value = quarterStrings[quarterStrings.indexOf(currentQuarter)-1]+' FY'+previousYear;
        quarters.push(obj1);
        
        var obj2 = {};
        obj2.label = currentQuarter+' FY'+previousYear.charAt(previousYear.length-2)+previousYear.charAt(previousYear.length-1);
        obj2.value = currentQuarter+' FY'+previousYear;
        quarters.push(obj2);
        
        for(var i=0; i<quarterStrings.length; i++){
            var obj ={};
            if(quarterStrings[i] != currentQuarter){
                console.log('If i: '+(i+1));
                obj.label = quarterStrings[i]+' FY'+currentYear.charAt(currentYear.length-2)+currentYear.charAt(currentYear.length-1);
                obj.value = quarterStrings[i]+' FY'+currentYear;
                quarters.push(obj);
            } else{
                console.log('Else i: '+(i+1));
                continue;
            }
        }        */
