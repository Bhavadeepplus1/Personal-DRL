({
    initRecords : function(component, event, helper) {
        ...
        else if(component.get("v.record").Name == 'Red Oak Sourcing')
    ...
    showExternalFam : function(component, event, helper){
        ...
        var jsonData = [];
        jsonData.push({label:'True Opportunity',count:''+reviewProductsObj.totalTrueOptyFam});//,totalVal:''+component.get("v.totalSalesTrueOpty")});//
        jsonData.push({label:'Awarded',count:''+reviewProductsObj.totalAwardedFam});//,totalVal:''+component.get("v.totalSalesAwarded")});
        jsonData.push({label:'In Bid',count:''+reviewProductsObj.totalInbidFam});//,totalVal:''+component.get("v.totalSalesInBid")});
        jsonData.push({label:'Not An Oportunity',count:''+reviewProductsObj.totalNotOptyFam});//,totalVal:''+component.get("v.totalSalesNotTrueOpty")});
        var countOfCon = parseInt(reviewProductsObj.totalPriceConFam)+parseInt(reviewProductsObj.totalSupplyConFam);
        jsonData.push({label:'DRL Constraint',count:''+countOfCon});//,totalVal:''+component.get("v.totalSalesCons")});
    ...
    showInternalFam : function(component, event, helper){
        ...
        jsonData.push({label:'In Bid',count:''+reviewProductsObj.totalInbidFam});//,totalVal:''+component.get("v.totalSalesInBid")});
        jsonData.push({label:'Not An Oportunity',count:''+reviewProductsObj.totalNotOptyFam});//,totalVal:''+component.get("v.totalSalesNotTrueOpty")});
        jsonData.push({label:'Price Constraint',count:''+reviewProductsObj.totalPriceConFam});//,totalVal:''+component.get("v.totalSalesPriceCons")});
        jsonData.push({label:'Supply Constraint',count:''+reviewProductsObj.totalSupplyConFam});//,totalVal:''+component.get("v.totalSalesSupplyCons")});
        component.set("v.ltngChartData",jsonData);
        component.set("v.showProdStatChart",true);
        component.set("v.showNdcList",false);
        component.set("v.showReviewProductsPopup",false);
    ...
    showExternalNDC : function(component, event, helper){
        ...
        if(!component.get("v.accObj").Vision_is_SRx_Account__c)
            jsonData.push({label:'In Bid',count:''+component.get("v.inBidCount"), dollar:0});//,totalVal:''+component.get("v.totalSalesInBid")});
    ...
    updatedSelectedProds : function(component, event, helper){
        ...
        if(selectedNdcs != undefined && selectedNdcs.length > 0){ 
            console.log('selectedNDCs --> '+JSON.stringify(selectedNdcs));
