({
    ...
    getCalculations : function(component,event,helper, currentValue, fieldName,isGuidancePrice,isMarketingPrice,isSalesPrice){ 
        ...
        var availableUnits = LineItem.Phoenix_Direct_Cordant__c!=undefined ? LineItem.Phoenix_Direct_Cordant__c : 0;
        var proposedDirectUnits = LineItem.Phoenix_Proposed_Direct_Selling_Unit__c!=undefined ? LineItem.Phoenix_Proposed_Direct_Selling_Unit__c : 0;
        var proposedIndirectUnits = LineItem.Phoenix_Proposed_Indirect_Selling_Unit__c!=undefined ? LineItem.Phoenix_Proposed_Indirect_Selling_Unit__c : 0;
        console.log('--------LineItemId---1-------'+LineItemId);
        console.log('-------otbDirIndir-----'+otbDirIndir);
        ...
        if((WAC < proposedPriceSales || WAC < proposedPriceMarketing) && !isGuidancePrice){
            ...
            component.set("v.singleRec.Phoenix_Wholesaler_Sales_Price__c", null);
                ...
                component.set("v.singleRec.Phoenix_Wholesaler_Diff_Price_Indirect__c", null);
                    ...
                    else if((LineItem.Phoenix_Direct_Cordant__c!=0 && LineItem.Phoenix_Direct_Cordant__c!=undefined  && (availableUnits<(proposedDirectUnits+proposedIndirectUnits)))){
                        var toastEvent = $A.get("e.force:showToast");
                        toastEvent.setParams({
                            "type":"Error",
                            "title": "Error",
                            "message": 'Proposed units can not be greater than Available Inventory Units ('+LineItem.Phoenix_Direct_Cordant__c+')'
                        });
                        toastEvent.fire();  
                        if(otbDirIndir=='Indirect'){
                            component.set("v.singleRec.Phoenix_Proposed_Indirect_Selling_Unit__c", null);
