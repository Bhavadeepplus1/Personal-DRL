({
    ...
    getBidDetailsViewPage: function (component,event,helper) {
        console.log('---------2------');
        let pageReference = component.get("v.pageReference");
        let recordId = pageReference.state.c__recordId;
        if (recordId != null && recordId != undefined && recordId != '') {
        ...
        getQuoteInfo.setCallback(this, function (response) {
            ...
            if (actState === 'SUCCESS') {
                ...
                {
                    component.set("v.wrap", response.getReturnValue());
                    console.log('-------------component.---------'+component.get('v.wrap.bid.Phoenix_Approval_Status__c'));
                      if(component.get('v.wrap.bid.Phoenix_Approval_Status__c')!='Draft') {
                        component.set('v.showDraftView1',false);
                    }
                    else{
                      component.set('v.showDraftView1',true);  
                    }
   ...
   loadBidRelatedLineItems: function (component, helper) {
        ...
        getProductsAction.setCallback(this, function (response) {
            var actState = response.getState();
            console.log('------response.getReturnValue()-'+JSON.stringify(response.getReturnValue()));
            //console.log('---length---response.getReturnValue()-'+(response.getReturnValue()).length);
            if (actState === 'SUCCESS') {
    ...
    onsearchDirectorhelper: function (component, helper) {
        ...
        if ((SearchKeyWordPD != null && SearchKeyWordPD != undefined &&SearchKeyWordPD != '' )) {
            ...
            getProductsAction.setParams({ pItem: allRecords, SearchKeyWordPD:SearchKeyWordPD,Family: searchFamily,searchName:searchText,RxSrxList:component.get("v.RxSrxList"),"quotewrap": component.get("v.wrap")});  
    ...
    searchTablehelper: function (component,helper) {
        ...
        if ((searchFilter != null && searchFilter != undefined && searchFilter != '' )) {
            ...
            getProductsAction.setParams({ pItem: allRecords,SearchKeyWordPD:SearchKeyWordPD, search: searchFilter,family: searchFamily ,RxSrxList:component.get("v.RxSrxList"),"quotewrap": component.get("v.wrap") });  
