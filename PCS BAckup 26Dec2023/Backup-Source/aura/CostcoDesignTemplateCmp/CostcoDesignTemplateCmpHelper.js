...
({
    ...
    getAllTotalValues: function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            if (response.getState() == "SUCCESS") {
                ...
                component.set("v.TMASTotal", responseValue[0].actSaDir12Months);
                component.set("v.CICP", responseValue[0].cicp);
                component.set("v.DEADNET", responseValue[0].deadnet);
                //component.set("v.TPTPercTotal",responseValue[0].TPMarginPerc);
    ...
    convertArrayOfObjectsToCSV : function(component,objectRecords,template,bidType,isReBid){
        ...
        if(bidType != "Customer Rebate Change" && bidType != "New Product Launch" && bidType != "Price Change" && bidType != "Sales Out Rebate"){
                ...
                for (var i = 0; i < resultlength; i++) {
                    if (component.get("v.BidAprrovalStatus") != 'Draft' && (ResultData[i].Phoenix_Product_Director__c == component.get("v.loggedInUserName") || (delegatedUser != null && delegatedUser.includes(ResultData[i].Phoenix_Product_Director__c))) && ResultData[i].Phoenix_ProposedContractBidPriceMktng__c == null && bidtype != 'Customer Rebate Change' && bidtype != 'Volume Review Only' && bidtype != 'Sales Out Rebate') {
                        if (ResultData[i].Phoenix_Marketing_Approval__c != 'Not Approved') {
                            qtyError = 'Please Enter Indirect Price to be loaded @ABC & McKesson ' + ResultData[i].Phoenix_Product__r.Name;
                        ...
                        if (ResultData[i].Phoenix_Marketing_Approval__c != 'Not Approved') {
                            qtyError = 'Please Enter Indirect Net Price: '+ ResultData[i].Phoenix_Product__r.Name;
                            console.log('marketing Price');
                            break;
                        } else {
                            break;
                        ...
                        else if(component.get("v.BidAprrovalStatus") != 'Draft' && (ResultData[i].Phoenix_Product_Director__c == component.get("v.loggedInUserName") || (delegatedUser != null && delegatedUser.includes(ResultData[i].Phoenix_Product_Director__c))) && ResultData[i].Phoenix_ProposedContractBidPriceMktng__c < ResultData[i].Phoenix_Wholesaler_Diff_Price_Indirect__c && bidtype!='Volume Review Only'){
                            if (ResultData[i].Phoenix_Marketing_Approval__c != 'Not Approved') {
                                qtyError = 'Indirect Price to be loaded @ABC & McKesson can not be less than Indirect net: '+ ResultData[i].Phoenix_Product__r.Name;
                                console.log('marketing Price');
                                break;
                            } else {
                                break;
