({
                               ...
                               if(response.getState()=="SUCCESS"){
                                   ...
                                   for(var i=0; i<mapListKeys.length; i++){
                                       ...
                                       for(var j=0; j<eachCustData.length; j++){
                                           ...
                                           if(eachCustData[j].Vision_Processed_Date__c >= response.lastDateRecent && eachCustData[j].Vision_Processed_Date__c <= response.firstDateRecent){
                                               if(sales.Vision_Sales__c != null)
                                                   SalesDollar += sales.Vision_Sales__c;
                                           ...
                                           } else if(eachCustData[j].Vision_Processed_Date__c >= response.lastDatePrevious && eachCustData[j].Vision_Processed_Date__c <= response.firstDatePrevious){
                                               if(sales.Vision_Sales__c != null)
                                                   SalesDollarPrev += sales.Vision_Sales__c;
                                               if(sales.Vision_Units__c != null)
                                     ...
                                     component.set("v.res",accWrapMap);
                                   if(accWrapMap.length != 0){
                                       ...
                                       for (var key in response.finalPrevMapSummary){
                                           ...
                                           if(comparison != 'lastquartervspreviousquarter'){
                                               ...
                                               //var label1 = key.split('@');
                                               //var label = label1[2].split('-')[0];
                                               if(!xAxisLabelsSummary.includes(label)){
                                                   xAxisLabelsSummary.push(label);
                                               }  
                                           } else{
                                       ...
                                       console.log('xAxisLabelsSummary--->'+xAxisLabelsSummary)
                                   ...
                                   component.set("v.loaded", false);
                                   console.log("Error in get data: "+JSON.stringify(response.getError()));
                               }
                           });
    ...
    getExternalData: function(component, event, helper,showInt){
        ...
        var action = component.get("c.getDRLSalesExternalData");
        action.setParams({
            selections: selections,
            selectedComparison: component.get("v.comparison"),
            customerId: component.get("v.customerId"),
        ...
        action.setCallback(this, function(response) 
                           {
                               if(response.getState()=="SUCCESS"){
                                   ...
                                   component.set("v.keySet", response.keySet);
                                   component.set("v.keySetPrev", response.keySetPrev);
                                   var mapList = response.mapListExt;  
                                   var mapListKeys = Object.keys(mapList);
                                   var accWrapMap = [];
                                   for(var i=0; i<mapListKeys.length; i++){
                                       var SalesDollar = 0; var SalesDollarPrev = 0; var Units = 0; var UnitsPrev = 0; var TPTDollar =0; var TPTDollarPrev =0;
                                       ...
                                       for(var j=0; j<eachCustData.length; j++){
                                           ...
                                           if(eachCustData[j].Vision_Processed_Date__c >= response.lastDateRecent && eachCustData[j].Vision_Processed_Date__c <= response.firstDateRecent){
                                               if(sales.Vision_Sales__c != null)
                                                   SalesDollar += Math.round((sales.Vision_Sales__c)*100)/100;
                                               if(sales.Vision_Units__c != null)
                                                   Units += sales.Vision_Units__c;
                                           } else if(eachCustData[j].Vision_Processed_Date__c >= response.lastDatePrevious && eachCustData[j].Vision_Processed_Date__c <= response.firstDatePrevious){
                                               if(sales.Vision_Sales__c != null)
                                                   SalesDollarPrev += Math.round((sales.Vision_Sales__c)*100)/100;
                                               if(sales.Vision_Units__c != null)
                                                   UnitsPrev += sales.Vision_Units__c;
                                           }
                                           custName = sales.Vision_Customer__r.Name;
                                   ...
                                   if(accWrapMap.length != 0){
                                       ...
                                       var somofUnitsRecent = 0;
                                       for (var key in response.finalRecentMapSummary){
                                           console.log('recent sales-->'+response.finalRecentMapSummary[key]['salesMonthlySummary'])
                                           salesdlSummary.push(response.finalRecentMapSummary[key]['salesMonthlySummary']);
                                           unitsSummary.push(response.finalRecentMapSummary[key]['unitsMonthlySummary']);
                                           if(comparison != 'lastquartervspreviousquarter'){
                                                console.log('key -->'+key)
                                               ...
                                               //var sales_recent = Math.round(num * 100) / 100
                                               sumofSalesRecent += Math.round((response.finalRecentMapSummary[key]['salesMonthlySummary'])*100)/100;
                                               somofUnitsRecent +=response.finalRecentMapSummary[key]['unitsMonthlySummary'];
                                           }
                                       }
                                       ...
                                       sumOfRecentSales.push(sumofSalesRecent);
                                       sumOfRecentUnits.push(somofUnitsRecent);
                                        console.log('sum of recent sales-->'+sumOfRecentSales)
                                       component.set("v.sumOfRecentSales",sumOfRecentSales);
                                       component.set("v.sumOfRecentUnits",sumOfRecentUnits);
                                       component.set("v.salesdlSummary",salesdlSummary);
                                       component.set("v.unitsSummary",unitsSummary);                                
    ...
    getChartData: function(component, event, helper,showInt,recentLastMonth,previousLastMonth){
        ...
        if(selectedComparison == 'last12monthsvsprevious12months'){
            console.log('in helpeer selection--'+component.get("v.selections"))
        ...
        action.setCallback(this, function(response) 
                           {
                               if(response.getState()=="SUCCESS"){
                                   ...
                                   console.log('responess-->'+JSON.stringify(response))
                                   component.set("v.loaded", false);
                               } else{
                                   component.set("v.loaded", false);
                                   console.log("Error in getting chart data: "+JSON.stringify(response.getError()));
                               }
                           });
        $A.enqueueAction(action); 
    },
