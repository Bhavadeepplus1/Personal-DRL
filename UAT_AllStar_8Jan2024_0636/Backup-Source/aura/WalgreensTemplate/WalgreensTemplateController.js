({
    ...
    submitFor : function(component, event, helper) { 
        ...
        }else if((bidtype == 'Product Addition'||bidtype == 'New Product Launch' || bidtype == 'RFP Bids') && (bidrecord.Phoenix_Initial_Order_Discount_of_Days__c == null || bidrecord.Phoenix_Initial_Order_Discount_of_Days__c== '' || bidrecord.Phoenix_Initial_Order_Discount_of_Days__c == 0 ) && bidrecord.Phoenix_Initial_Order_Discount_Type__c != 'None'){
           var iodPerc=bidrecord.Phoenix_Initial_Order_Discount_of_Days__c;
            if(iodPerc==null || iodPerc==''){
                isBidNotValid='Please Enter Initial Order Discount # of Days';
            }else{
                isBidNotValid= iodPerc < 1 ? 'Initial Order Discount # of Days must be greater than 0' : 'Please Enter Initial Order Discount # of Days';
            }
        }else if((bidtype == 'Product Addition' ||bidtype == 'New Product Launch'|| bidtype == 'RFP Bids') && (bidrecord.Phoenix_Proposed_Initial_Order_Discount__c == null || bidrecord.Phoenix_Proposed_Initial_Order_Discount__c== '' || bidrecord.Phoenix_Proposed_Initial_Order_Discount__c== 0) && bidrecord.Phoenix_Initial_Order_Discount_Type__c != 'None'){
            var iodPerc=bidrecord.Phoenix_Proposed_Initial_Order_Discount__c;
            if(iodPerc==null || iodPerc==''){
                isBidNotValid='Please Enter Proposed Initial Order Discount %';
            }else{
                isBidNotValid= iodPerc < 1 ? 'Proposed Initial Order Discount % must be greater than 0' : 'Please Enter Proposed Initial Order Discount %';
            }
        }
        if(isBidNotValid){
            ...
            action.setCallback(this, function(response) 
                               {
                                   if(response.getState()=="SUCCESS"){
                                       ...
                                       for(var i=0;i<bidlines.length;i++){
                                            ...
                                            if((templateType=='Walgreens' || templateType=='ABC Progen') && bidtype!='Price Change' && bidtype!='Customer Rebate Change' && bidtype !='Sales Out Rebate' && bidlines[i].Phoenix_Proposed_Indirect_Selling_Unit__c==null){
                                   ...
                                   if(qtyError){
                                       ...
                                       if(!component.get("v.winnabilityIgnored")){
                                           if(templateType=='Walgreens' && bidtype == 'RFP Bids' && (component.get("v.BidAprrovalStatus") == 'SCM &amp; Marketing' || component.get("v.BidAprrovalStatus") == 'Marketing')){
                                               ...
                                               if(BidLineItemListAll != null){
                                                   for(var i=0; i<BidLineItemListAll.length; i++){
                                                       if(BidLineItemListAll[i].Phoenix_Product__r.Winnability__c != null && BidLineItemListAll[i].Phoenix_Product__r.Winnability__c < 80){
                                                           lowWinnabilityScoreList.push(BidLineItemListAll[i]);
                                                           totalCOLO += (BidLineItemListAll[i].Phoenix_Net_Sales_Internal__c != null ? BidLineItemListAll[i].Phoenix_Net_Sales_Internal__c : 0);
                                                       }
                                                   }
                                               }
    ...
    handleOnload : function(component, event, helper) {      
        ...
        console.log('pVip--'+pVip);
                if(recUi.record.fields["Phoenix_Proposed_Value_Est_VIP__c"] != null && recUi.record.fields["Phoenix_Proposed_Value_Est_VIP__c"] != undefined){
        crntVip=recUi.record.fields["Phoenix_Current_Value_Est_VIP__c"].value;
                }
    ...
    onRecordSuccess : function(component, event, helper) {
        ...
        var rebateHeader , feeHeader, marketingHeader,cdHeader, vipHeader;
        ...
        if (helper.requiredValidation(component, event)){
            ...
            action.setCallback(this, function(response) {
              ...
              //  component.set("v.vipHeader",false);
            });
            $A.enqueueAction(action);
        }
         helper.getAllTotalValues(component, event, helper);
