({
    ...
    insertBidLineItems : function(component, event, helper, bidRec, offsetVal){
        ...
        console.log('component.get("v.syncToAnalysis") ---> '+component.get("v.syncToAnalysis"));
        var action = component.get("c.addBidLinesToBid");
        action.setParams({
            bidRec : bidRec,
            optyProdList : sendingList,
            bidType : component.get("v.selectedBidType"),
            syncToAnalysis : component.get("v.syncToAnalysis")
        });
        action.setCallback(this, function(response){
            if(response.getState() == 'SUCCESS'){
                ...
                if(!respMessage.includes('ERROR')){
                    if(offsetVal > selectedProdListSize){
                        if(component.get("v.syncToAnalysis"))
                            helper.syncAnalysis(component, event, bidRec, helper);
                        else
                            helper.redirectMethod(component, event, bidRec, helper);
                    }
                    else{
    ...
    redirectMethod : function(component, event, bidRec, helper){
                                        ...
                                        urlEvent.setParams({
                                            ...
                                            urlEvent.setParams({
                                                ...
                                                urlEvent.setParams({
                                                    ...
                                                    else if(bidRec.Phoenix_Customer_Type__c=='Humana Indirect retail' || bidRec.Phoenix_Customer_Type__c=='Humana Indirect CII'){
                                                        urlEvent.setParams({
                                                            "url": "/lightning/n/Humana?c__id="+bidRec.Id
                                                        }); 
                                                    }
                                                    else
                                                    {
    ...
    syncAnalysis : function(component, event, bidRec, helper){
        var sendingList = component.get("v.selectedProdList");
        var action = component.get("c.updateInAnalysisClass");
        action.setParams({
            optyProdList : sendingList,
        ...
        action.setCallback(this, function(response){
            if(response.getState() == 'SUCCESS'){
                ...
                toastEvent.setParams({
                    "type": "SUCCESS",
                    "title": "UPDATED!",
                    "message": '100% analysis has been updated..'
                });
                toastEvent.fire();
                helper.redirectMethod(component, event, bidRec, helper);
                component.set('v.showProductsloading', false);
                ...
                toastEvent.setParams({
                    "type": "ERROR",
                    "title": "FAILED!",
                    "message": 'Failed to update 100% analysis..'
                });
                toastEvent.fire();
                helper.redirectMethod(component, event, bidRec, helper);
                component.set('v.showProductsloading', false);
