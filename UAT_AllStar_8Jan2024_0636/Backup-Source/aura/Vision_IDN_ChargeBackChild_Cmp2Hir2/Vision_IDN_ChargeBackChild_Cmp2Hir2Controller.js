({
        ...
        child1relatedList.forEach(function(rec){
            ...
            var units = (rec.T2_contract_units_paid_qty__c != null || rec.T2_contract_units_paid_qty__c != undefined) ? rec.T2_contract_units_paid_qty__c : 0;
                                       if(rec.T2_Month_Year__c == component.get("v.firstColumnKey")){
                                           firstColSalesSummary += Math.round((salesPrice + Number.EPSILON) * 100) / 100;//Math.round(rec.T2_Contract_Sales_Indirect_Price__c);
                                           firstColUnitsSummary +=  Math.round(units);
                                           child1PFamilyList.push(rec.T2_revised_product_family__c);
                                       }
	...
	handleClick1: function(component, event, helper){
       ...
       var child1relatedList = component.get("v.record.child1RelatedList");
        ...
        var allData= [];
       child1relatedList.forEach(function(rec){
           console.log('test child cmp2==>'+rec.T2_Child2_Name__c);
           if(child2mapObj.hasOwnProperty(rec.T2_Child2_Name__c)){
                 
               var relatedList = child2mapObj[rec.T2_Child2_Name__c];
               relatedList.push(rec);
               child2mapObj[rec.T2_Child2_Name__c] = relatedList;
