...
({
    ...
    submitFor: function (component, event, helper) {
        ...
        if(bidrecord.Phoenix_Reference_Contracts__c==null && bidType!='New Customer'){
         ...
         /*else if((bidType == 'Product Addition' || bidType == 'RFP Bids' || bidType == 'New Customer') && (bidrecord.Phoenix_Initial_Order_Discount_Type__c==null || bidrecord.Phoenix_Initial_Order_Discount_Type__c=='')){  
            isBidNotValid= "Please Select Initial Order Discount Type";
            
        }
        
            else if((bidType == 'Product Addition' || bidType == 'RFP Bids' || bidType == 'New Customer') && (bidrecord.Phoenix_Initial_Order_Discount_of_Days__c == null || bidrecord.Phoenix_Initial_Order_Discount_of_Days__c== '' || bidrecord.Phoenix_Initial_Order_Discount_of_Days__c == 0 ) && bidrecord.Phoenix_Initial_Order_Discount_Type__c != 'None'){
        ...
        }else if((bidType == 'Product Addition' || bidType == 'RFP Bids' || bidType == 'New Customer') && (bidrecord.Phoenix_Proposed_Initial_Order_Discount__c == null || bidrecord.Phoenix_Proposed_Initial_Order_Discount__c== '' || bidrecord.Phoenix_Proposed_Initial_Order_Discount__c== 0) && bidrecord.Phoenix_Initial_Order_Discount_Type__c != 'None'){
            ...
            action.setCallback(this, function (response) {
                if (response.getState() == "SUCCESS") {
                    ...
                    for (var i = 0; i < bidlines.length; i++) {
                        if ((templateType == 'Direct' || templateType == 'Direct and Indirect') && bidType  != 'Price Change' && bidType != 'Customer Rebate Change' && bidType != 'Sales Out Rebate' && bidlines[i].Phoenix_Proposed_Direct_Selling_Unit__c == null) {
                            qtyError = 'Please Enter All Proposed Direct Selling Units';
                            console.log('direct');
                            break;
                        } 
                        //else if ((templateType == 'Humana Indirect CII' || templateType == 'Humana Indirect retail') && bidType != 'Price Change' && bidlines[i].Phoenix_Proposed_Indirect_Selling_Unit__c == null) {
                       else if (bidType != 'Volume Review Only' && bidType != 'Sales Out Rebate' && component.get("v.isMarketingApprovePerson") == component.get("v.loggedInUserName") && component.get("v.loggedInUserName") == bidlines[i].Phoenix_Product_Director1__c && bidlines[i].Phoenix_Proposed_Indirect_Selling_Unit__c == null) { 
                        qtyError = 'Please Enter Proposed Indirect Selling Units for '+bidlines[i].Phoenix_Product__r.Name;
                            console.log('in direct');
                            break;
                        } 
                            /*else if (bidType != 'Volume Review Only' && component.get("v.isMarketingApprovePerson") == undefined && bidlines[i].Phoenix_ProposedContract_Bid_Price_Sales__c == null) {
                            qtyError = 'Please Enter Sales Price';
                            console.log('Sales Price');
                            break;
                        } */
                        else if (templateType == 'Humana Indirect CII' && (bidType != 'Volume Review Only' && bidType != 'Sales Out Rebate' && component.get("v.isMarketingApprovePerson") == component.get("v.loggedInUserName") && component.get("v.loggedInUserName") == bidlines[i].Phoenix_Product_Director1__c && bidlines[i].Phoenix_ProposedContractBidPriceMktng__c == null)) {
                            qtyError = 'Please Enter Proposed Direct Contract Price';
                            break;
                        }
                            else if (bidType != 'Volume Review Only' && bidType != 'Sales Out Rebate' && component.get("v.isMarketingApprovePerson") == component.get("v.loggedInUserName") && component.get("v.loggedInUserName") == bidlines[i].Phoenix_Product_Director1__c && bidlines[i].Phoenix_Wholesaler_Diff_Price_Indirect__c == null) {
                            qtyError = 'Please Enter Proposed Indirect Contract Price';
                                console.log('bid type is==>'+bidType)
                               
                            console.log('marketing Priceers');
                            break;
                        }
                    ...
                    for (var i = 0; i < bidlines.length; i++) {
                         if (bidType != 'New Product Launch' && (bidlines[i].Phoenix_Override_Current_Indirect_Units__c == null && (bidlines[i].Phoenix_Current_Indirect_Selling_Unit__c == 0 || bidlines[i].Phoenix_Current_Indirect_Selling_Unit__c == undefined || bidlines[i].Phoenix_Current_Indirect_Selling_Unit__c == null))){
        ...
        action.setCallback(this, function (response) {
            if (response.getState() == "SUCCESS") {
                ...
                lineItemsList.forEach(function (line) {
                    productCodes.push(line.Phoenix_Product__r.ProductCode);
                });              
            ...
            //helper.updateLatestPriceMethod(component, event, helper);  
           console.log('latest bds==>'+component.get("v.latestBidNames"))
          console.log('bidRecord.Humana_Latest_Bids__c'+bidRecord.Humana_Latest_Bids__c)
                if (bidRecord.Humana_Latest_Bids__c != null && bidRecord.Humana_Latest_Bids__c != 'undefined') {
                    var refBids = bidRecord.Humana_Latest_Bids__c;
                    console.log('in iff==>'+refBids)
                    component.set("v.latestBidNames", refBids.split(','));
                }else{
                    
                    var latestBidNames = [];
                    var linteItemlist = component.get("v.BidLineItemListAll");
                    for(var i=0;i<= linteItemlist.length;i++){
                        if(linteItemlist[i] != undefined){
                          console.log('line item list==>'+JSON.stringify(linteItemlist[i]))
                          if(linteItemlist[i].hasOwnProperty('Latest_Line_Item__r')){
                      
                        console.log('in else test=='+linteItemlist[i].Latest_Line_Item__r.Phoenix_Bid__r.Name)
                        //linteItemlist[i].Latest_Line_Item__r.Phoenix_Bid__r.Name;
                        if(!latestBidNames.includes(linteItemlist[i].Latest_Line_Item__r.Phoenix_Bid__r.Name)){
                            latestBidNames.push(linteItemlist[i].Latest_Line_Item__r.Phoenix_Bid__r.Name)
                        }
                          }
                        }
                    } 
                     component.set("v.latestBidNames",latestBidNames);
           			 component.set("v.selectedBids",latestBidNames);
                }
                //var a = component.get('c.savePriceDetails');
        	//$A.enqueueAction(a); 
    ...
    onRecordSubmit: function (component, event, helper) {
        ...
        var slctBids = component.get("v.latestBidNames");
        console.log('slctBids==>'+slctBids)
        eventFields["Humana_Latest_Bids__c"] = slctBids.toString();
        /* var cntrctstring='';
        ...
        if(slctCntrcts.length==0 && component.get("v.BidTypeVal")!='New Customer'){
            ...
            //component.set("v.latestBidNames", slctBids);
            
             let newListContracts = [];
             if(slctCntrcts != undefined && slctCntrcts !=null && slctCntrcts.length>0){
                 slctCntrcts.forEach(function(contrct){
                     newListContracts.push(contrct.trim());
                 });
             }
            if (component.get("v.BidTypeVal") != 'New Customer') {
                console.log('slctCntrcts-->'+newListContracts)
                helper.getNPRDataOfContracts(component, event, helper, newListContracts, component.get("v.templateType"));
            }
        }
        /*by satya*/
    ...
    onRecordSuccess: function (component, event, helper) {
        ...
        var rebateHeader, feeHeader, marketingHeader,cashTermsPer;
        var rebatetype = component.find("headerRebateType");
        var cashTerms = component.find("cashTermsId");
        ...
        if($A.util.isArray(cashTerms)){
            cashTerms=cashTerms[0];
        }
		...
		if(cashTerms != null){
            cashTermsPer = cashTerms.get("v.value");
        }else{
            cashTermsPer= null;
        }
        ...
        if (helper.requiredValidation(component, event)) {
            ...
            action.setParams({
                ...
                'cashTerms':cashTermsPer,
                'isCDChanged': component.get("v.isCDChanged")
            });
            action.setCallback(this, function (response) {
        ...
        //var a = component.get('c.savePriceDetails');
        			//$A.enqueueAction(a);
        if(component.get("v.isRemovedPill")){
      		helper.savePriceDetails1(component, event,helper);
        }
        // helper.searchProductFamilyChange(component, event,helper);
    },
    ...
    hideModel: function (component, event, helper) {
      ...
      hideBidModel: function (component, event, helper) {
        component.set("v.showModal", false);
    ...
    saveToProceedContracts: function (component, event, helper) {
       ...
       showBidsModel: function (component, event, helper) {
        var LineItemtable = component.find("LineTable");
        console.log('modalstart---' + LineItemtable);
        $A.util.removeClass(LineItemtable, "maintable");
        component.set("v.showBidModal", true);
        component.set('v.bidsColumns', [{
                label: 'Bid No',
                fieldName: 'linkName',
                type: 'url',
            ...
            {
                label: 'Bid Name',
                fieldName: 'Phoenix_Bid_Name__c',
                type: 'Text'
            },
            {
                label: 'Bid Type',
                fieldName: 'Phoenix_Bid_Type__c',
                type: 'Picklist'
            },
            {
                label: 'Approval Status',
                fieldName: 'Phoenix_Approval_Status__c',
                type: 'Picklist'
            },
            {
                label: 'Bid Template',
                fieldName: 'Phoenix_Customer_Type__c',
                type: 'Picklist'
            },
            {
                label: 'Customer',
                ...
                type: 'url',
                typeAttributes: {label: { fieldName: 'Customer_Name__c' }, target: '_blank'}
            },
            {
                label: 'Created By',
                fieldName: 'CreatedBy',
        ...
        console.log('--bidCustomer--' + bidCustomer);
        if (bidCustomer != null && bidCustomer != undefined) {
            helper.fetchBids(component, event, helper, bidCustomer);
        } else {
            component.set("v.bidsList", null);
        }
    }, 
     showProductPositionModel: function (component, event, helper) {
      ...
      saveBidDetails: function (component, event, helper) {
    ...
    savePriceDetails: function (component, event, helper) {
        ...
        for (var i = 0; i < selectedBids.length; i++) {
            ...
            //console.log('test id==>'+selectedBids[i].ID);
        }
        component.set("v.latestBidNames",bidNames)
        component.set("v.showSaveCancelBtn", true);
         console.log('bid names==>'+bidNames)
         component.set("v.showBidModal", false);
    ...
    savePosDetails: function (component, event, helper) {
     ...
     clearBid: function (component, event, heplper) {
        component.set("v.isRemovedPill",true)
        var selectedPillId = event.getSource().get("v.name");
         console.log('selectedPillId==>'+selectedPillId);
        var AllPillsList = component.get("v.latestBidNames");//component.get("v.selectedBids");

        for (var i = 0; i < AllPillsList.length; i++) {
            if (AllPillsList[i] == selectedPillId) {
                AllPillsList.splice(i, 1);
                component.set("v.selectedBids", AllPillsList);
                component.set("v.latestBidNames", AllPillsList);
            }
        }
         console.log('latestBidNames >>=='+component.get("v.latestBidNames"))
        component.set("v.showSaveCancelBtn", true);
