...
({
            ...
            action.setCallback(this, function (response) {
                if (response.getState() == "SUCCESS") {
                    ...
                    for (var i = 0; i < bidlines.length; i++) {
                        ...
                        if(contrcts.length>0 && (contrcts.includes("3000000589"))&& bidType!='Price Change' && bidType!='Customer Rebate Change' && bidType !='Sales Out Rebate' && (bidlines[i].Phoenix_ProposedDirectAholdDelhaizeUnits__c==null  )){
                                               qtyError='Please Enter Proposed Auto Sub Units for '+bidlines[i].Phoenix_Product__r.Name;
                     ...
                     for (var i = 0; i < bidlines.length; i++) {
                          // for ovveride units
                            if ((bidType != 'New Product Launch' && bidType != 'New Customer')&& (bidlines[i].Phoenix_Override_Current_Direct_Units__c == null && (bidlines[i].Phoenix_Current_Wholesaler_Units__c == 0 || bidlines[i].Phoenix_Current_Wholesaler_Units__c == undefined || bidlines[i].Phoenix_Current_Wholesaler_Units__c == null))){
                           isOverrideUnitsavl = true;
                            break;
                        }
                         else if ((bidType != 'New Product Launch' && bidType != 'New Customer')&& (bidlines[i].Phoenix_Override_Current_Indirect_Units__c == null && (bidlines[i].Phoenix_Current_Anda_Units__c == 0 || bidlines[i].Phoenix_Current_Anda_Units__c == undefined || bidlines[i].Phoenix_Current_Anda_Units__c == null))){
                           isOverrideUnitsavl = true;
                            break;
                        }
                         else if ((bidType != 'New Product Launch' && bidType != 'New Customer')&& (bidlines[i].Phoenix_Override_Current_Units__c == null && (bidlines[i].Phoenix_Current_Retail_Direct_Units__c == 0 || bidlines[i].Phoenix_Current_Retail_Direct_Units__c == undefined || bidlines[i].Phoenix_Current_Retail_Direct_Units__c == null))){
                           isOverrideUnitsavl = true;
                            break;
                        }
 ...
 /*Product Position Logic*/
    ...
    showSaveCancel: function (component, event, helper) {
        var isCdChanged;
        if(event.getSource() != null && event.getSource() != undefined)
        isCdChanged = event.getSource().get('v.fieldName') == 'Phoenix_Current_CD__c' ? true : false;
    ...
    downloadCsv: function (component, event, helper) {
        ...
        action.setCallback(this, function (a) {
            ...
            hiddenElement.download = 'DSH Calculation View' + '-' + bidNumber + '-' + bidName + '-' + Now + '.csv'; // CSV file Name* you can change it.[only name not .csv] 
