            ...
            action.setCallback(this, function(response) 
                               {
                                   if(response.getState()=="SUCCESS"){
                                       ...
                                       console.log('bidlines length==>'+bidlines.length)
                                       console.log('json res==>'+JSON.stringify(bidlines))
                                       for(var i=0;i<bidlines.length;i++){
                                           console.log('indirectCons'+indirectCons.length)
                                           ...
                                           if(indirectCons.length>0 && bidType!='Price Change' && bidType!='Customer Rebate Change' && bidType !='Sales Out Rebate' && (bidlines[i].Phoenix_Indirect_ESI__c==null  && bidlines[i].Phoenix_Indirect_Kroger__c==null  && bidlines[i].Phoenix_Indirect_Rx_Outreach__c==null  && bidlines[i].Phoenix_Indirect_Supervalu__c==null && bidlines[i].Phoenix_Indirect_Cigna__c==null && bidlines[i].Phoenix_Indirect_Cordant__c==null && bidlines[i].Phoenix_Indirect_Accerodo__c==null &&  bidlines[i].Phoenix_Others_Indirect__c==null )){
                                               ...
                                               console.log('in Indirect test');
                                               isOverrideUnitsavl = false;
                                               break;
                                           }
                                           if(bidlines[i].Phoenix_Wholesaler_Diff_Price_Indirect__c==null && component.get("v.isMarketingApprovePerson")==component.get("v.loggedInUserName") && (component.get("v.loggedInUserName")== bidlines[i].Phoenix_Product_Director1__c || (delegatedUser != null && delegatedUser.includes(bidlines[i].Phoenix_Product_Director1__c))) && bidType!='Customer Rebate Change' && bidType!='Volume Review Only' && bidType !='Sales Out Rebate'){
                                               qtyError='Please Enter Wholesaler Difference Price (Indirect)';
                                  ...
                                  /*      else if (bidType == 'New Product Launch' && bidlines[i].Phoenix_Internal_Dead_Net_Price__c < bidlines[i].Phoenix_Lowest_Price_SKU__c && bidlines[i].Phoenix_Internal_Dead_Net_Price__c != 0){
                                     ...
                                     /*   else if (bidType == 'New Product Launch' && bidlines[i].Phoenix_Internal_Dead_Net_Price__c < bidlines[i].Phoenix_Lowest_Price_SKU__c && bidlines[i].Phoenix_Internal_Dead_Net_Price__c != 0 && (delegatedUser != null && delegatedUser.includes(bidlines[i].Phoenix_Product_Director1__c))){
                                       ...
                                       for(var i=0;i<bidlines.length;i++){
                                           ...
                                           else if (bidType != 'New Product Launch' && (bidlines[i].Phoenix_Override_Current_Direct_Units__c == null && (bidlines[i].Phoenix_Current_Direct_Selling_Unit__c == 0 || bidlines[i].Phoenix_Current_Direct_Selling_Unit__c == undefined || bidlines[i].Phoenix_Current_Direct_Selling_Unit__c == null))){
                            					isOverrideUnitsavl = true;
                                               	break;
                        					}
