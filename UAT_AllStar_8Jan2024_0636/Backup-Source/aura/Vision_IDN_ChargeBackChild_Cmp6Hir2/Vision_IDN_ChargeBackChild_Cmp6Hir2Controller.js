...
    */    
        ...
        for(var i=0 ; i<allData.length ; i++){
            ...
            if(keyList[i] == component.get("v.firstColumnKey")){
                ...
                octchargeBackAmount = octchargeBackAmount+Math.round(((rec.T2_Contract_Sales_Indirect_Price__c) + Number.EPSILON) * 100) / 100;//Math.round(rec.Contract_Sales_Indirect_Price__c);
            ...
            if(keyList[i] == component.get("v.secondColumnKey")){
                sampleList.forEach(function(rec){
                novchargeBackAmount = novchargeBackAmount+Math.round(((rec.T2_Contract_Sales_Indirect_Price__c) + Number.EPSILON) * 100) / 100;//Math.round(rec.Contract_Sales_Indirect_Price__c);
                    col2Units = col2Units+Math.round(rec.T2_contract_units_paid_qty__c);
                 });
            }
             if(keyList[i] == component.get("v.thirdColumnKey")){
                sampleList.forEach(function(rec){
                decchargeBackAmount = decchargeBackAmount+Math.round(((rec.T2_Contract_Sales_Indirect_Price__c) + Number.EPSILON) * 100) / 100;//Math.round(rec.Contract_Sales_Indirect_Price__c);
            ...
            if(keyList[i] == component.get("v.fourthColumnKey")){
                 ...
                 sampleList.forEach(function(rec){
                     ...
                     janchargeBackAmount = janchargeBackAmount+Math.round(((rec.T2_Contract_Sales_Indirect_Price__c) + Number.EPSILON) * 100) / 100;//Math.round(rec.Contract_Sales_Indirect_Price__c);
                     col4Units = col4Units+Math.round(rec.T2_contract_units_paid_qty__c);
                 });
            }
            if(keyList[i] == component.get("v.fifthColumnKey")){
                sampleList.forEach(function(rec){
                febchargeBackAmount = febchargeBackAmount+Math.round(((rec.T2_Contract_Sales_Indirect_Price__c) + Number.EPSILON) * 100) / 100;//Math.round(rec.Contract_Sales_Indirect_Price__c);
                    col5Units = col5Units+Math.round(rec.T2_contract_units_paid_qty__c);
                 });
            }
            if(keyList[i] == component.get("v.sixthColumnKey")){
                sampleList.forEach(function(rec){
                marchchargeBackAmount = marchchargeBackAmount+Math.round(((rec.T2_Contract_Sales_Indirect_Price__c) + Number.EPSILON) * 100) / 100;//Math.round(rec.Contract_Sales_Indirect_Price__c);
