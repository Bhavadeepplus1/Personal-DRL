...
({
    doInit: function (component, event, helper) {
        ...
        wholesalerPrice = (component.get("v.singleRec.Phoenix_Current_Indirect_Price__c")/component.get("v.singleRec.Phoenix_Wholesaler_Diff_Price_Indirect__c")*100).toFixed(2) + '%';
    ...
    calcGoalSeek: function (component, event, helper) {
        ...
        let deadnetPrice = component.get("v.expectDdeadnet");
         console.log('VIP  $--->' +component.get("v.singleRec.Phoenix_Proposed_Est_VIP__c"));
        ...
        requiredPricedeNumer = 1 - (vipperunit - (currentcashterms + propadminfee+proposedrebatepercentage) * vipperunit) - proposedrebatepercentage- currentcashterms - propadminfee;
    ...
    calcCustDeadNetGoalSeek: function (component, event, helper) {
        ...
        let proposedrebatepercentage1 = parseFloat(component.get("v.singleRec.Phoenix_Proposed_Current_Rebate__c") / 100);
        let proposedrebatepercentage = Number.isFinite(proposedrebatepercentage1) ? proposedrebatepercentage1 : 0;
        
        let currentcashterms = bidRecord.Phoenix_Current_CD__c;
        currentcashterms = (currentcashterms === undefined || currentcashterms == null) ?0: currentcashterms;
        if (currentcashterms != null) {
            currentcashterms = parseFloat(currentcashterms / 100);
        }
        ...
        let requiredPriceNumer = deadnetPrice;
        let requiredPricedeNumer;
        requiredPricedeNumer = 1 - (vipperunit - (currentcashterms + propadminfee+proposedrebatepercentage) * vipperunit);
        console.log('requiredPricedeNumer next--->' + requiredPricedeNumer);
        component.set("v.RequiredPrice", requiredPriceNumer / requiredPricedeNumer);
