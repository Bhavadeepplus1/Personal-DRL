...
({
    ...
    clickHandler : function(component,event,helper){
        alert('You just clicked on TPT Grid Image');
    },
                ...
                if (response.getState() == "SUCCESS") {
                    ...
                    for (var i = 0; i < bidlines.length; i++) {
                        if ((templateType == 'Direct' || templateType == 'Direct and Indirect') && bidType  != 'Price Change' && bidType != 'Customer Rebate Change' && bidType != 'Sales Out Rebate' && bidlines[i].Phoenix_Proposed_Direct_Selling_Unit__c == null) {
                            ...
                            console.log('direct');
                            break;
                        } else if ((templateType == 'Indirect' || templateType == 'Direct and Indirect') && bidType != 'Price Change' && bidType != 'Customer Rebate Change' && bidType != 'Sales Out Rebate' && bidlines[i].Phoenix_Proposed_Indirect_Selling_Unit__c == null) {
                            ...
                            else if (bidType != 'Volume Review Only' && bidType != 'Sales Out Rebate' && component.get("v.isMarketingApprovePerson") == component.get("v.loggedInUserName") && component.get("v.loggedInUserName") == bidlines[i].Phoenix_Product_Director1__c && bidlines[i].Phoenix_ProposedContractBidPriceMktng__c == null) {
                    ...
                    for (var i = 0; i < bidlines.length; i++) {
                        // for ovveride units
                                    if ( (templateType == 'Direct' || templateType == 'Direct and Indirect')&&(bidType != 'New Product Launch' && bidType != 'New Customer')&& (bidlines[i].Phoenix_Override_Current_Direct_Units__c == null && (bidlines[i].Phoenix_Current_Direct_Selling_Unit__c == 0 || bidlines[i].Phoenix_Current_Direct_Selling_Unit__c == undefined || bidlines[i].Phoenix_Current_Direct_Selling_Unit__c == null))){
                                            isOverrideUnitsavl = true;
                                            break;
                                        }
                                            else if ( (templateType == 'Indirect' || templateType == 'Direct and Indirect')&&(bidType != 'New Product Launch' && bidType != 'New Customer')&& (bidlines[i].Phoenix_Override_Current_Indirect_Units__c == null && (bidlines[i].Phoenix_Current_Indirect_Selling_Unit__c == 0 || bidlines[i].Phoenix_Current_Indirect_Selling_Unit__c == undefined || bidlines[i].Phoenix_Current_Indirect_Selling_Unit__c == null))){
                                                isOverrideUnitsavl = true;
                                                break;
                                            }
    ...
    initRecords: function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            if (response.getState() == "SUCCESS") {
                ...
                component.set("v.accountNumber", bidRecord.Phoenix_Customer__r.AccountNumber);
    ...
    onRecordSubmit: function (component, event, helper) {
        ...
        if(slctCntrcts.length==0 && component.get("v.BidTypeVal")!='New Customer'){
            ...
            if (component.get("v.BidTypeVal") != 'New Customer' && exstingContracts !== slctCntrcts.toString()) {
    ...
    onRecordSuccess: function (component, event, helper) {
        ...
        var rebateHeader, feeHeader, marketingHeader,vipHeader;
         var viptype = component.find("ProValEstId");
        if($A.util.isArray(viptype)){
            viptype=viptype[0];
        }
