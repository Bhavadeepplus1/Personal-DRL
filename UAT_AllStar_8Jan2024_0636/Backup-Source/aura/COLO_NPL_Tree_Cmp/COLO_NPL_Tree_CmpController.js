({
    doInit: function(component, event, helper){
        ...
        var NPLDataList = component.get("v.NPLDataList");
        ...
        if(NPLDataList != null){
            for(var i=0; i<NPLDataList.length; i++){
                var awardedQty = ((NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Awarded_Quantity__c != null) ? NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Awarded_Quantity__c : 0);
                var proposedASP = ((NPLDataList[i].Phoenix_Proposed_ASP_Dose__c != null) ? NPLDataList[i].Phoenix_Proposed_ASP_Dose__c : 0);
                var proposedQty = ((NPLDataList[i].Phoenix_Final_Total_Selling_Unit__c != null) ? NPLDataList[i].Phoenix_Final_Total_Selling_Unit__c : 0);
                var awardedSales = awardedQty*proposedASP;                 
                var currentSales = 0;
                if(NPLDataList[i].Phoenix_Bid_Template_Refrence__c == 'ClarusOne')
                    currentSales = (NPLDataList[i].Phoenix_Net_Sales_External__c != null ? NPLDataList[i].Phoenix_Net_Sales_External__c : 0) ;
                else
                    currentSales = (NPLDataList[i].Finance_Current_Sales__c != null ? NPLDataList[i].Finance_Current_Sales__c : (NPLDataList[i].Phoenix_Current_Sales_Finance__c != null ? NPLDataList[i].Phoenix_Current_Sales_Finance__c : 0) );
                var proposedSales = (NPLDataList[i].Phoenix_Proposed_Sales__c != null ? NPLDataList[i].Phoenix_Proposed_Sales__c : 0);
                var awardedTPT = ((NPLDataList[i].Phoenix_Total_DRL_Share__c != null) ? NPLDataList[i].Phoenix_Total_DRL_Share__c : 0);
                var proposedTPT = ((NPLDataList[i].Phoenix_Total_DRL_Share__c != null) ? NPLDataList[i].Phoenix_Total_DRL_Share__c : 0);
                var currentTPT = (NPLDataList[i].Phoenix_Current_TP_Margin__c != null ? NPLDataList[i].Phoenix_Current_TP_Margin__c : 0);
                ...
                NPLDataList[i].awardedQty = awardedQty;
                NPLDataList[i].proposedASP = proposedASP;
                NPLDataList[i].currentSales = currentSales;
                NPLDataList[i].proposedQty = proposedQty;
                NPLDataList[i].awardedSales = (awardedSales - currentSales);
                NPLDataList[i].awardedTPT = (awardedTPT - currentTPT);
                NPLDataList[i].proposedTPT = (proposedTPT - currentTPT);
                NPLDataList[i].awardedTPTPercent = awardedTPTPercent;
                NPLDataList[i].proposedTPTPercent = proposedTPTPercent;
                NPLDataList[i].proposedSales = (proposedSales-currentSales);
                if(NPLDataList[i].Customer_Response_Lines__r != null && NPLDataList[i].Customer_Response_Lines__r.length > 0 && NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Bid_Status__c == 'Awarded'){
                    optyWonList.push(NPLDataList[i]);
                    totalOptyWonSales += (awardedSales-currentSales);
                    totalOptyWonTPTSales += (awardedTPT - currentTPT);
                    if(NPLDataList[i].Phoenix_Product__r.Phoenix_Is_partner_product__c){
                        if(NPLDataList[i].Phoenix_DRL_Margin_DRL__c < 25) {
                            wonLessThresholdList.push(NPLDataList[i]);
                            wonLessThresholdSales += (awardedSales-currentSales);
                            wonLessThresholdTPTSales += (awardedTPT - currentTPT);
                            if(NPLDataList[i].CIP_Status_Internal_Use__c == 'OnGoing'){
                            	wonCIPOnList.push(NPLDataList[i]);    
                                wonCIPOnSales += (awardedSales-currentSales);
                                wonCIPOnTPTSales += (awardedTPT - currentTPT);
                            } else{
                            	wonCIPInitiatedList.push(NPLDataList[i]);    
                                wonCIPInitiatedSales += (awardedSales-currentSales);
                                wonCIPInitiatedTPTSales += (awardedTPT - currentTPT);
                            }
                        } else {
                            wonGreaterThresholdList.push(NPLDataList[i]);
                            wonGreaterThresholdSales += (awardedSales-currentSales);
                            wonGreaterThresholdTPTSales += (awardedTPT - currentTPT);
                        }
                    } 
                    else {
                        if(NPLDataList[i].Phoenix_DRL_Margin_DRL__c < 40) {
                            wonLessThresholdList.push(NPLDataList[i]);
                            wonLessThresholdSales += (awardedSales-currentSales);
                            wonLessThresholdTPTSales += (awardedTPT - currentTPT);
                            if(NPLDataList[i].CIP_Status_Internal_Use__c == 'OnGoing'){
                ...
                else if(NPLDataList[i].Customer_Response_Lines__r != null && NPLDataList[i].Customer_Response_Lines__r.length > 0 && NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Bid_Status__c == 'Declined by Customer'){
                    optyLostList.push(NPLDataList[i]);
                    if(NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Lead_Time_Days__c == 0){
                        priceCOLOList.push(NPLDataList[i]);
                        totalPriceCOLOSales += (proposedSales-currentSales);
                        totalPriceCOLOTPTSales += (proposedTPT - currentTPT);
                        if(NPLDataList[i].Phoenix_Product__r.Phoenix_Is_partner_product__c){
                            if(NPLDataList[i].Phoenix_DRL_Margin_DRL__c < 25) {
                                lostLessThresholdList.push(NPLDataList[i]);
                                lostLessThresholdSales += (proposedSales-currentSales);
                                lostLessThresholdTPTSales += (proposedTPT - currentTPT);
                                if(NPLDataList[i].CIP_Status_Internal_Use__c == 'OnGoing'){
                                    lostCIPOnList.push(NPLDataList[i]);    
                                    lostCIPOnSales += (proposedSales-currentSales);
                                    lostCIPOnTPTSales += (proposedTPT - currentTPT);
                                } else{
                                    lostCIPInitiatedList.push(NPLDataList[i]);    
                                    lostCIPInitiatedSales += (proposedSales-currentSales);
                                    lostCIPInitiatedTPTSales += (proposedTPT - currentTPT);
                                }
                            } else {
                                lostGreaterThresholdList.push(NPLDataList[i]);
                        ...
                        supplyCOLOList.push(NPLDataList[i]);
                        totalSupplyCOLOSales += (proposedSales-currentSales);
                        totalSupplyCOLOTPTSales += (proposedTPT - currentTPT);
                        if(NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Lead_Time_Days__c == null || NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Lead_Time_Days__c == 0 ||
                          NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Lead_Time_Days__c == 7 || NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Lead_Time_Days__c == 15 ||
                          NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Lead_Time_Days__c == 20 || NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Lead_Time_Days__c == 30){
                            ltOneSales += (proposedSales-currentSales);
                            ltOneTPTSales += (proposedTPT - currentTPT);
                        } else if(NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Lead_Time_Days__c == 30 || NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Lead_Time_Days__c == 45){
                            ltTwoSales += (proposedSales-currentSales);
                            ltTwoTPTSales += (proposedTPT - currentTPT);
                        } else if(NPLDataList[i].Customer_Response_Lines__r[0].Phoenix_Lead_Time_Days__c == 60){
    ...
    optyWonTimeSeries : function(component, event, helper){
        component.set("v.selectedTimeSeriesItem",'New Product Launch Opportunity Won Time Series');
        component.set("v.showTimeSeries",true);
    },
    optyLostTimeSeries : function(component, event, helper){
        component.set("v.selectedTimeSeriesItem",'New Product Launch Opportunity Lost Time Series');
        component.set("v.showTimeSeries",true);
    },
