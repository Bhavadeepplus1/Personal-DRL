({
	...
	getFullData : function(component, event, helper) {
        ...
        console.log('selectedUserId testhh==??'+component.get("v.selectedUserId"));
        console.log('userRegion...'+JSON.stringify(component.get("v.userRegion")));
      
       component.set("v.isSpinnerLoad", true);
		var action = component.get("c.getDatafromTab2SummaryBatch");
        action.setParams({
                                  ...
                                  // console.log('Success currentSixMonthKeys==>'+responseList[i].currentSixMonthKeys[0]);
                                   ...
                                   responseList[i].idnchargebackList.forEach(function(rec){
                                      //userRegion = rec.Region__c;
                                    	categoryList.push(rec.T2_Category__c);
                                       if(rec.T2_Month_Year__c == responseList[i].currentSixMonthKeys[5]){//oct-22
                                          
                                           firstColSalesSummary += Math.round(rec.T2_Contract_Sales_Indirect_Price__c);
                                           firstColUnitsSummary +=  Math.round(rec.T2_contract_units_paid_qty__c);
                                       }
                                       if(rec.T2_Month_Year__c == responseList[i].currentSixMonthKeys[4]){
                                           secColSalesSummary += Math.round(rec.T2_Contract_Sales_Indirect_Price__c);
                                           secColUnitsSummary +=  Math.round(rec.T2_contract_units_paid_qty__c);
                                       }
                                       if(rec.T2_Month_Year__c == responseList[i].currentSixMonthKeys[3]){
                                           thirdColSalesSummary += Math.round(rec.T2_Contract_Sales_Indirect_Price__c);
                                           thirdColUnitsSummary +=  Math.round(rec.T2_contract_units_paid_qty__c);
                                       }
                                       if(rec.T2_Month_Year__c == responseList[i].currentSixMonthKeys[2]){
                                           fourthColSalesSummary += Math.round(rec.T2_Contract_Sales_Indirect_Price__c);
                                           fourthColUnitsSummary +=  Math.round(rec.T2_contract_units_paid_qty__c);
                                       }
                                       if(rec.T2_Month_Year__c == responseList[i].currentSixMonthKeys[1]){
                                           fifthColSalesSummary += Math.round(rec.T2_Contract_Sales_Indirect_Price__c);
                                           fifthColUnitsSummary +=  Math.round(rec.T2_contract_units_paid_qty__c);
                                       }
                                       if(rec.T2_Month_Year__c == responseList[i].currentSixMonthKeys[0]){
                                           sixthColSalesSummary += Math.round(rec.T2_Contract_Sales_Indirect_Price__c);
                                           sixthColUnitsSummary +=  Math.round(rec.T2_contract_units_paid_qty__c);
                                       }
                                   });
    ...
    convertArrayOfObjectsToCSV: function(component,objectRecords){
      ...
      chargebackList.forEach(function(eachRec){
          ...
          relatedList.forEach(function(chargeBackRec){
              var salesPrice = (chargeBackRec.T2_Contract_Sales_Indirect_Price__c != null && chargeBackRec.T2_Contract_Sales_Indirect_Price__c != undefined) ? chargeBackRec.T2_Contract_Sales_Indirect_Price__c : 0;
              var units = (chargeBackRec.T2_contract_units_paid_qty__c != null && chargeBackRec.T2_contract_units_paid_qty__c != undefined) ? chargeBackRec.T2_contract_units_paid_qty__c : 0;
      ...
      console.log('testing result listOfWrappers--->'+JSON.stringify(listOfWrappers));
        ...
        myMap.set("Sales", "Contract_Sales_Indirect_Price__c");
        myMap.set("Units", "contract_units_paid_qty__c");
     ...
     /*   var productList=[];
      for(var i=0;i<objectRecords.length;i++){
           objectRecords[i].idnchargebackList.forEach(function(rec){
          productList.push(rec.material_description__c);
           });
      }
        console.log('productList'+productList);   */                                            
        
        //console.log('idnChargeBackList'+JSON.stringify(idnChargeBackList));
     	
        listOfWrappers.forEach(function(eachREc){
            var idnChargeBackList = eachREc.eachHsptl.idnchargebackList;
            console.log('length of idn chargeback==>'+idnChargeBackList.length)
            ...
            for (let [key, value] of myMap) {
                if(counter > 0){ 
                    ...
                    if(value=='hospitalName'){
                        csvStringResult += '"'+ idnChargeBackList[i].T2_Hospital_Name__c +'"';
                        //objectRecords[i]["Phoenix_Product__r"]["Name"]
                    }
                    else if(value =='Child1_Name__c'){
                        csvStringResult += '"'+ idnChargeBackList[i].T2_Child1_Name__c +'"';
                    }
                	 else if(value =='Child2_Name__c'){
                        csvStringResult += '"'+ idnChargeBackList[i].T2_Child2_Name__c +'"';
                    }
                	else if(value =='revised_product_family__c'){
                        csvStringResult += '"'+ idnChargeBackList[i].T2_revised_product_family__c +'"';
                    }
               		 else if(value =='material_description__c'){
                        csvStringResult += '"'+ idnChargeBackList[i].T2_material_description__c +'"';
                    }
                	else if(value =='Contact_Description__c'){
                        csvStringResult += '"'+ idnChargeBackList[i].T2_Contact_Description__c +'"';
                    }else if(value =='Month_Year__c'){
                        csvStringResult += '"'+ idnChargeBackList[i].T2_Month_Year__c +'"';
                    }
                    else if(value == 'Contract_Sales_Indirect_Price__c'){
                        if(idnChargeBackList[i].T2_Month_Year__c == 'Jan-23'){
                            console.log('contracts sales==>'+idnChargeBackList[i].T2_Contract_Sales_Indirect_Price__c)
                          csvStringResult += '"'+ idnChargeBackList[i].T2_Contract_Sales_Indirect_Price__c +'"';
                        	console.log('in second')
                        }else if(idnChargeBackList[i].T2_Month_Year__c == 'Feb-23'){
                          csvStringResult += '"'+ idnChargeBackList[i].T2_Contract_Sales_Indirect_Price__c +'"';
                        }else if(idnChargeBackList[i].T2_Month_Year__c == 'Mar-23'){
                          csvStringResult += '"'+ idnChargeBackList[i].T2_Contract_Sales_Indirect_Price__c +'"';
                        }else if(idnChargeBackList[i].T2_Month_Year__c == 'Apr-23'){
                          csvStringResult += '"'+ idnChargeBackList[i].T2_Contract_Sales_Indirect_Price__c +'"';
                        }else if(idnChargeBackList[i].T2_Month_Year__c == 'May-23'){
                          csvStringResult += '"'+ idnChargeBackList[i].T2_Contract_Sales_Indirect_Price__c +'"';
                        }else if(idnChargeBackList[i].T2_Month_Year__c == 'Jun-23'){
                          csvStringResult += '"'+ idnChargeBackList[i].T2_Contract_Sales_Indirect_Price__c +'"';
                        }
                        
                    }else if(value == 'contract_units_paid_qty__c'){
                        if(idnChargeBackList[i].T2_Month_Year__c == 'Jan-23'){
                            console.log('contracts sales==>'+idnChargeBackList[i].T2_contract_units_paid_qty__c)
                          csvStringResult += '"'+ idnChargeBackList[i].T2_contract_units_paid_qty__c +'"';
                        	console.log('in second')
                        }else if(idnChargeBackList[i].T2_Month_Year__c == 'Feb-23'){
                          csvStringResult += '"'+ idnChargeBackList[i].T2_contract_units_paid_qty__c +'"';
                        }else if(idnChargeBackList[i].T2_Month_Year__c == 'Mar-23'){
                     ...
                     csvStringResult += '"'+ idnChargeBackList.AverageSales +'"';
                     
                    }
                else if(value == 'AverageUnits'){
                     csvStringResult += '"'+ idnChargeBackList.AverageUnits +'"';
                     
                    }
                 else if(value == 'totalSales'){
                     csvStringResult += '"'+ idnChargeBackList.totalSales +'"';
                     
                    }
                else if(value == 'totalUnits'){
                     csvStringResult += '"'+ idnChargeBackList.totalUnits +'"';
