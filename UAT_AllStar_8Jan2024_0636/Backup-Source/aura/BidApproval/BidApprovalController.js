        ...
        action.setCallback(this, function(response) 
                           {
                               if(response.getState()=="SUCCESS"){
                                   ...
                                   console.log('finalList length==>'+JSON.stringify(finalList))
                                    var hoverList =[];
                                   for(var i=0;i<finalList.length;i++){
                                       hoverList.push(finalList[i].Logged_in_User__c)
                                       
                                   }
                                   console.log('hoverList test'+hoverList)
                                  var hoverMsg='';
                                   for(var i=0;i<hoverList.length;i++){
                                     hoverMsg = hoverMsg +hoverList[i]+','
                                   }
                                    var hoverMsg = hoverMsg.substring(0, hoverMsg.length - 1);
                                   hoverMsg = hoverMsg +' have visited the Bid'
                                   	console.log('each string==>'+hoverMsg);
                                    component.set('v.showApproversForTeam',hoverMsg);
                                   }else{
                                     component.set('v.showApproversForTeam','No one visit the Bid');  
                                   }
        ...
        action.setCallback(this, function(response) 
                           {
                               if(response.getState()=="SUCCESS"){
                                    var response =  response.getReturnValue();
                                   console.log('Approvers who visited the Bid==>'+JSON.stringify(response))
                                   var finalList=[];
                                   if(response.length > 0){
                                   for(var i=0;i<response.length;i++){
                                        finalList.push(response[i].recList[0])
                                  ...
                                  var hoverMsg='';
                                   ...
                                   hoverMsg = hoverMsg +' have visited the Bid'
                                   	console.log('each string==>'+hoverMsg);
                                    component.set('v.lastActivityTime2',hoverMsg);
                                   }else{
                                       component.set('v.lastActivityTime2','No one visit the Bid');
                                   }
                                  console.log('finalList'+JSON.stringify(finalList));
                                   let str = "Hello World!\nThis is my string";
