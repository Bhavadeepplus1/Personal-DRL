({
        ...
        action.setCallback(this, function(response) 
                           {
                               if(response.getState()=="SUCCESS"){
                                  ...
                                  console.log('keySet in helper==>'+response.keySet)
                                  var keySet = ["May-21","Jun-21","Jul-21","Aug-21","Sep-22","Oct-21","Nov-21","Dec-21","Jan-22","Feb-22","Mar-22","Apr-22"]
                                  component.set("v.keySet", keySet);
                                  // component.set("v.keySet", response.keySet);
                                   ...
                                   console.log('accMap==>'+JSON.stringify(accMap).length);
                                   for(var i=0; i<response.salesList.length; i++){
                                       if(response.salesList[i].parentAccName == null || response.salesList[i].parentAccName == ''||response.salesList[i].parentAccName == undefined){
                                           response.salesList[i].showItem = true;
                                       }else{
                                           response.salesList[i].showItem = false;
                                       }
                                   }
                                  // console.log('show==>'+response.salesList[i].showItem)
                                        for(var i=0; i<accMap.length; i++){
                                            accMap[i].showItem = false;
                                        }
                                  /* response.salesList.forEach(function(item){
                                       item.childSalesList.forEach(function(child){ 
                                           
                                           response.salesList.salesdl += parseFloat(child.salesdl != undefined ? child.salesdl : 0 ); 
                                           response.salesList.units += parseFloat(child.units != undefined ? child.units : 0 ); 
                                           
                                           response.salesList.SalesDollarPrev += parseFloat(child.SalesDollarPrev != undefined ? child.SalesDollarPrev : 0 ); 
                                           response.salesList.UnitsPrev += parseFloat(child.UnitsPrev != undefined ? child.UnitsPrev : 0 ); 
                                       });
                                   });*/
                                 ...
                                 /*  var parentList=[];
                                   for(var i=0;i<response.accWrapMap.length;i++){
                                       parentList.push(response.accWrapMap[i].parentName);
                                   }
    ...
    buildData : function(component, event, helper, response) {
        ...
        if(component.get("v.isINTChecked")){
         responseValue = response.salesList;
          ...
          responseValue = response.accWrapMap;  
        }
        var summaryObj = {};
       ...
       if(component.get("v.isINTChecked")){
        ...
        if(component.get("v.isINTChecked")){
            ...
            for(var i=0; i<responseValue.length; i++){
            console.log('Sales Dollar 1: '+responseValue[i].salesdl);
            var trendingSales;
            var trendingVolume;
            var trendingTPT;
            var trendingTPTPercent;
            if(parseInt(responseValue[i].SalesDollarPrev) == 0 || parseInt(responseValue[i].salesdl) == 0){
                trendingSales = 0;
            } else{
                trendingSales = (parseInt(responseValue[i].salesdl)/parseInt(responseValue[i].SalesDollarPrev))-1;
            }
            if(parseInt(responseValue[i].UnitsPrev) ==0 || parseInt(responseValue[i].units) == 0){
                trendingVolume = 0;
            } else{
            ...
            salesPrevSummary = salesPrevSummary + Math.round(responseValue[i].SalesDollarPrev);
            console.log('salesPrevSummaryyyy-->'+salesPrevSummary);
            salesSummary = salesSummary + Math.round(responseValue[i].salesdl);
            salesTrendingSummary = (salesSummary/salesPrevSummary)-1;
       ...
       if(showTPT){
        ...
        if(comparison == 'lastquartervspreviousquarter'){
            ...
            if(showTPT){
                console.log('sumof recent sales==>'+component.get("v.sumOfRecentSales"))
            ...
            if(showTPT){
               ...
               console.log('x axis values-->'+component.get("v.xAxisLabelsSummary"));
                console.log('unit values-->'+component.get("v.unitsPrevSummary"));
            chartobj1 = new Chart(ctx2, {
    ...
    getChartData: function(component, event, helper,showInt,recentLastMonth,previousLastMonth){
        console.log('showInt in getchartData');
        component.set('v.loaded', true);
