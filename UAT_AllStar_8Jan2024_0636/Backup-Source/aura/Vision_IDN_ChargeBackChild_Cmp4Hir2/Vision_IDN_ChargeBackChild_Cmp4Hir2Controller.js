({
     doInit:function(component, event, helper){
        ...
        contractDescriptionList.forEach(function(rec){
            ...
            var units = (rec.T2_contract_units_paid_qty__c != null || rec.T2_contract_units_paid_qty__c != undefined) ? rec.T2_contract_units_paid_qty__c : 0;
                                       if(rec.T2_Month_Year__c == component.get("v.firstColumnKey")){
                                           ...
                                           allPFamilyList.push(rec.T2_revised_product_family__c);
                                       }
        ...
        component.set("v.avgofUnitsSummary",avgofUnitsSummary);
         component.set("v.allmaterialDescriptionlist",allmaterialDescriptionlist);
          component.set("v.allPFamilyList",allPFamilyList);
	...
	handleClick1: function(component, event, helper){
       ...
       contractDescRelatedList.forEach(function(rec){
           ...
           console.log('test child cmp2==>'+rec.T2_revised_product_family__c);
           if(productFamilymapObj.hasOwnProperty(rec.T2_revised_product_family__c)){
                 console.log('inside if child cmp2==>'+rec.T2_revised_product_family__c);
               var relatedList = productFamilymapObj[rec.T2_revised_product_family__c];
               relatedList.push(rec);
               productFamilymapObj[rec.T2_revised_product_family__c] = relatedList;
