({
	initRecords : function(component, event, helper) {
        ...
        var productRelatedList = component.get("v.record.productRelatedList");
        component.set("v.materialDescRelatedList",productRelatedList);
        console.log('productRelatedList==>'+JSON.stringify(productRelatedList))
        ...
        component.set("v.productFamily",productFamily);
         var monthyearmapObj ={};
        var allData= [];
         productRelatedList.forEach(function(rec){
...
    */    
        ...
        for(var i=0 ; i<allData.length ; i++){
            ...
            var sampleList = allData[i]['monthyearRelatedList'];
             //var salesPrice = (rec.H1_Contract_Sales_Indirect_Price__c != null || rec.H1_Contract_Sales_Indirect_Price__c != undefined) ? rec.H1_Contract_Sales_Indirect_Price__c : 0;
              // var units = (rec.H1_contract_units_paid_qty__c != null || rec.H1_contract_units_paid_qty__c != undefined) ? rec.H1_contract_units_paid_qty__c : 0;
            ...
            if(keyList[i] == component.get("v.firstColumnKey")){
                ...
                col1Units = col1Units+Math.round(rec.contract_units_paid_qty__c);
                    allcontractsDescriptionlist.push(rec.Contact_Description__c);
                 });
            }
            if(keyList[i] == component.get("v.secondColumnKey")){
                sampleList.forEach(function(rec){
                novchargeBackAmount = novchargeBackAmount+Math.round(((rec.Contract_Sales_Indirect_Price__c) + Number.EPSILON) * 100) / 100;//Math.round(rec.Contract_Sales_Indirect_Price__c);
             ...
             if(keyList[i] == component.get("v.thirdColumnKey")){
                sampleList.forEach(function(rec){
                decchargeBackAmount = decchargeBackAmount+Math.round(((rec.Contract_Sales_Indirect_Price__c) + Number.EPSILON) * 100) / 100;//Math.round(rec.Contract_Sales_Indirect_Price__c);
            ...
            if(keyList[i] == component.get("v.fourthColumnKey")){
                 ...
                 sampleList.forEach(function(rec){
                     ...
                     janchargeBackAmount = janchargeBackAmount+Math.round(((rec.Contract_Sales_Indirect_Price__c) + Number.EPSILON) * 100) / 100;//Math.round(rec.Contract_Sales_Indirect_Price__c);
            ...
            if(keyList[i] == component.get("v.fifthColumnKey")){
                sampleList.forEach(function(rec){
                febchargeBackAmount = febchargeBackAmount+Math.round(((rec.Contract_Sales_Indirect_Price__c) + Number.EPSILON) * 100) / 100;//Math.round(rec.Contract_Sales_Indirect_Price__c);
            ...
            if(keyList[i] == component.get("v.sixthColumnKey")){
                sampleList.forEach(function(rec){
                marchchargeBackAmount = marchchargeBackAmount+Math.round(((rec.Contract_Sales_Indirect_Price__c) + Number.EPSILON) * 100) / 100;//Math.round(rec.Contract_Sales_Indirect_Price__c);
           ...
           avgsalesofSixMonth = (octchargeBackAmount + novchargeBackAmount + decchargeBackAmount + janchargeBackAmount+febchargeBackAmount+marchchargeBackAmount)/6;
        	...
        	totalSalesofSixMonth = Math.round((totalSalesofSixMonth + Number.EPSILON) * 100) / 100;
    ...
    handleClick1: function(component, event, helper){
       var prodRelatedList = component.get("v.record.productRelatedList");
         component.set("v.record.showcontractDescription", false);
        component.set("v.expandNDC", true);
        var contractmapObj ={};
        var allData= [];
        prodRelatedList.forEach(function(rec){
        ...
        console.log('child cmp4 contractmapObj===>'+JSON.stringify(contractmapObj));
        ...
        for(var i=0; i<keys.length; i++){
            ...
            obj.contract_Description__c = keys[i];
