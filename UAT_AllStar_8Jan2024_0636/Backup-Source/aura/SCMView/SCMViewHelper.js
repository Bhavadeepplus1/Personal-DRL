        ...
        action.setCallback(this, function (response){
            if(response.getState() === "SUCCESS"){
               ...
               console.log('scmDataList'+JSON.stringify(scmDataList) );
                ...
                var skuLength=0;var approvedcount=0;var rejectedcount=0;var nullcount=0;var isAllRejSKU=0; var BidTemplate = '';
                if(scmDataList.length>0){
                    ...
                    BidTemplate = scmDataList[0].bidLineObj.Phoenix_Bid_Template_Refrence__c;
                    component.set("v.bidRecord", scmDataList[0].bidRecord);
                    for(var i=0; i<scmDataList.length; i++){
                        if(scmDataList[i].bidLineObj.Phoenix_Net_Sales_Internal__c != null){
                           scmDataList[i].formattedNetSalesInternal = scmDataList[i].bidLineObj.Phoenix_Net_Sales_Internal__c.toLocaleString('en-US', { style: 'currency', currency: 'USD',  minimumFractionDigits: 0, maximumFractionDigits: 1 }); 
                        }
                    }
                }   
                component.set("v.updatedData",scmDataList);                
                component.set("v.bidTemplate", BidTemplate);
                //}
