({
    ...
    getCalculations: function (component, event, helper, currentValue, fieldName, isGuidancePrice, isMarketingPrice, isSalesPrice) {
        ...
        var NetIndirectPrice5=LineItem.Phoenix_Customer_Dead_net__c;
        //calc for new validation
        /*
        var wholeDiffPrIndirect = LineItem.Phoenix_Wholesaler_Diff_Price_Indirect__c != null ? LineItem.Phoenix_Wholesaler_Diff_Price_Indirect__c : LineItem.Phoenix_ProposedContract_Bid_Price_Sales__c != null ? LineItem.Phoenix_ProposedContract_Bid_Price_Sales__c : LineItem.Phoenix_Current_Indirect_Price__c != null ? LineItem.Phoenix_Current_Indirect_Price__c : 0;
        var CMFee = (LineItem.Phoenix_Contract_Mngment_Fee_Wholesaler__c != null ? LineItem.Phoenix_Contract_Mngment_Fee_Wholesaler__c : 0) / 100;
        var contractMngmtWhole = wholeDiffPrIndirect * CMFee;
        var ProposedDirectcontract = LineItem.Phoenix_ProposedContractBidPriceMktng__c != null ? LineItem.Phoenix_ProposedContractBidPriceMktng__c : 0;
        var IndirectCD = LineItem.Phoenix_INDIRECT_CD__c != null ? LineItem.Phoenix_INDIRECT_CD__c : 0;
        var DirectCD = LineItem.Phoenix_Direct_CD_Per_Unit__c != null ? LineItem.Phoenix_Direct_CD_Per_Unit__c : 0;
        
        var NetIndirectPrice1 = (ProposedDirectcontract + IndirectCD + contractMngmtWhole - DirectCD);
        var NetIndirectPrice=NetIndirectPrice1
        console.log('NetIndirectPrice'+NetIndirectPrice1);
        console.log('NetIndirectPrice'+NetIndirectPrice);
        console.log('diffPriceIndirect'+diffPriceIndirect);
        */
        ...
        if ((WAC != 0 && WAC != undefined) && (WAC < proposedPriceSales || WAC < proposedPriceMarketing) && !isGuidancePrice) {
            ...
            toastEvent.setParams({
                "type": "Error",
                "title": "Error",
                "message": 'Indirect Price can not be greater than WAC ($' + LineItem.Phoenix_WAC__c + ')'
            });
            toastEvent.fire();
            ...
            toastEvent.setParams({
                "type": "Error",
                "title": "Error",
                "message": 'Indirect Price can not be less than Marketing Price'
            });
            toastEvent.fire();
            component.set("v.singleRec.Phoenix_Wholesaler_Diff_Price_Indirect__c", null);
        }
        */
        /*
        else if (diffPriceIndirect < NetIndirectPrice && bidType != 'Volume Review Only') {
            var toastEvent = $A.get("e.force:showToast");
            toastEvent.setParams({
                "type": "Error",
                "title": "Error",
                "message": 'Indirect Price can not be less than Net Indirect Price'
            });
            toastEvent.fire();
            component.set("v.singleRec.Phoenix_Wholesaler_Diff_Price_Indirect__c", null);
        }
